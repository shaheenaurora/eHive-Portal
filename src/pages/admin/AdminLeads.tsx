      ...
      {q.data && q.data.length === 0 && (
        <div className="eh-card">
          <Empty
            ...
            p="Submissions land here within seconds of a form being sent."
          />
        </div>
      )}
                ...
                return (
                  ...
                  >
                    <td>
                      <b>{nm ?? l.email ?? "—"}</b>
                      {nm && l.email ? (
                        <div className="eh-muted eh-sm">{l.email}</div>
                      ) : null}
                    </td>
                    <td data-label="Form">
                      <Pill>{formLabel(l.form)}</Pill>
                    </td>
                    <td data-label="Detail" className="eh-sm">
                      {leadHighlight(l)}
                    </td>
                    ...
                    <td data-label="When" className="eh-sm eh-muted">
...
function LeadDetail({
  ...
  try {
    data = lead.payload ? JSON.parse(lead.payload) : {};
  } catch {
    /* ignore */
  }
  const report = buildScorecardReport(data);
  const bcSections =
    lead.form === "brand-check" ? brandCheckSections(data) : null;
  const str = (k: string) =>
    typeof data[k] === "string" && data[k] ? String(data[k]) : null;
  const contact: [string, string | null][] = [
    ["Name", str("name")],
    ["Email", lead.email ?? str("email")],
    ["Phone", str("phone")],
  ...
  return (
    <Modal
      title={`Lead — ${leadName(lead) ?? lead.email ?? formLabel(lead.form)}`}
      onClose={onClose}
      wide
    >
      ...
      {report && (
        ...
        >
          ...
          <p className="eh-sm eh-muted" style={{ margin: ".5rem 0 1rem" }}>
            {report.band.copy}
          </p>
            ...
            {report.domains.map(d => {
              ...
              return (
                <div
                  ...
                  style={{
                    display: "grid",
                    gridTemplateColumns: "9rem 1fr 3.2rem",
                    alignItems: "center",
                    gap: ".6rem",
                  }}
                >
                  <span
                    ...
                    style={{
                      ...
                      color: weak ? "var(--eh-red)" : "var(--eh-ink-2)",
                    }}
                  >
                    {d.key}
                    {weak ? " ⚠" : strong ? " ★" : ""}
                  ...
                  <span
                    ...
                    style={{
                      textAlign: "right",
                      color: weak ? "var(--eh-red)" : "var(--eh-mut)",
                    }}
                  >
                    {d.pct}%
                  </span>
              ...
              <div className="eh-list">
                {s.items.map((it, i) => (
                  <div
                    className="row"
                    key={i}
                    style={{ display: "block", padding: ".5rem 0" }}
                  >
                    ...
                    <div
                      ...
                      style={{ marginTop: ".15rem", whiteSpace: "pre-wrap" }}
                    >
                      {it.a}
                    </div>
                  </div>
      ...
      <div className="eh-list eh-mb">
        ...
        <div className="row">
          <span className="d">Form</span>
          <Pill>{formLabel(lead.form)}</Pill>
        </div>
        <div className="row">
          <span className="d">Source</span>
          <span className="t eh-sm">{lead.sourcePage ?? "—"}</span>
        </div>
        <div className="row">
          <span className="d">Captured</span>
          <span className="t eh-sm">{fmtDateTime(lead.createdAt)}</span>
        </div>
      </div>
...
function fmtVal(v: unknown): string {
  ...
  if (typeof v === "object") {
    ...
    return Object.entries(v as Record<string, unknown>)
      .map(
        ([k, val]) =>
          `${prettyKey(k)}: ${typeof val === "object" ? JSON.stringify(val) : String(val)}`
      )
      .join(" · ");
  }
  return String(v);
  ...
  return Object.entries(data)
    .filter(
      ([k, v]) =>
        !CONTACT_KEYS.has(k) &&
        !PLUMBING_KEYS.has(k) &&
        !SCORECARD_INTERNAL.has(k) &&
        !BRANDCHECK_INTERNAL.has(k) &&
        v !== "" &&
        v != null
    )
...
function brandCheckSections(
  data: Record<string, unknown>
): BrandCheckSection[] | null {
...
function leadHighlight(l: LeadRow): ReactNode {
  ...
  try {
    d = l.payload ? JSON.parse(l.payload) : {};
  } catch {
    /* ignore */
  }
  ...
  const pick = (k: string) =>
    typeof d[k] === "string" && d[k] ? String(d[k]) : null;
  if (l.form === "brand-check")
    return (
