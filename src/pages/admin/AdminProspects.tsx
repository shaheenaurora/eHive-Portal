...
export default function AdminProspects() {
  ...
  const q = trpc.adminEngage.prospects.useQuery(
  ...
  const followUps = trpc.adminEngage.followUps.useQuery(undefined, {
    retry: false,
  });
  ...
  return (
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      ...
      {(followUps.data?.length ?? 0) > 0 && (
        ...
        >
          <div className="eh-eyebrow" style={{ marginBottom: ".5rem" }}>
            Guest follow-ups due (CH-01/CH-03)
          </div>
              ...
              return (
                <div className="row" key={f.id}>
                  <div style={{ flex: 1 }}>
                    ...
                    {f.chapterName && (
                      <span className="eh-muted eh-sm"> · {f.chapterName}</span>
                    )}
                    {f.dueAt && (
                      <Pill color={overdue ? "red" : "gold"}>
                  ...
                  <span className="eh-row" style={{ gap: ".3rem" }}>
                    <button
                      ...
                      disabled={doneFollowUp.isPending}
                      onClick={async () => {
                        if (
                          await confirmDialog({
                      ...
                      onClick={async () => {
                        if (
                          await confirmDialog({
                            ...
                            body: "It won't count as completed.",
                            confirmLabel: "Dismiss",
                            danger: true,
                          })
                        )
                          doneFollowUp.mutate({ id: f.id, dismiss: true });
                      }}
                    >
                      Dismiss
                    </button>
      ...
      <div className="eh-between eh-mb">
        ...
        <button className="eh-btn gold sm" onClick={() => setAddOpen(true)}>
          Add prospect →
        </button>
      </div>
      ...
      {q.data && q.data.length === 0 && (
        <div className="eh-card">
          <Empty
            big="No prospects here yet."
            p="Log a prospect or an event guest to start the funnel."
          />
        </div>
      )}
            ...
            <tbody>
              {(q.data as Row[]).map(p => (
                ...
                >
                  ...
                  <td data-label="Contact" className="eh-sm">
                    {p.email ?? p.phone ?? "—"}
                  </td>
                  <td data-label="Company" className="eh-sm">
                    {p.company ?? "—"}
                  </td>
                  <td data-label="Stage">
                    <Pill color={STAGE_COLOR[p.stage]}>
                      {PROSPECT_STAGE_LABEL[p.stage]}
                    </Pill>
                  </td>
                  <td data-label="Source" className="eh-sm">
                    {p.source ?? "—"}
                  </td>
                  <td data-label="Added" className="eh-sm eh-muted">
                    {fmtDate(p.createdAt)}
                  </td>
                  <td>
    ...
    <Modal title="Add prospect" onClose={onClose}>
      ...
      <Field label="Source">
        <input
          ...
          value={f.source}
          onChange={set("source")}
          placeholder="e.g. Chapter meeting guest, referral, event"
        />
      </Field>
      <Field label="Notes">
        <textarea
          ...
          value={f.notes}
          onChange={set("notes")}
          placeholder="Context, who introduced them, what they're building…"
        />
      </Field>
      <button
        className="eh-btn gold"
        disabled={add.isPending || f.name.trim().length < 2}
        onClick={() =>
          add.mutate({
            name: f.name,
    ...
    <Modal title={p.name} onClose={onClose}>
      <div className="eh-list eh-mb">
        <div className="row">
          ...
          <span className="t">{p.email ?? "—"}</span>
        </div>
        <div className="row">
          <span className="d">Phone</span>
          <span className="t">{p.phone ?? "—"}</span>
        </div>
        <div className="row">
          <span className="d">Company</span>
          <span className="t">{p.company ?? "—"}</span>
        </div>
        <div className="row">
          <span className="d">Source</span>
          <span className="t">{p.source ?? "—"}</span>
        </div>
      </div>
      <Field label="Stage">
        <select
      ...
      <p className="eh-sm eh-muted" style={{ marginTop: 0 }}>
        Move through the funnel: Prospect → Guest (attended) → Invited to apply
        → Converted. When they're ready, invite them to the Application.
      </p>
      <Field label="Notes">
        <textarea
          className="eh-textarea"
