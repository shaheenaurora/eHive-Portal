...
export default function AdminFrp() {
  ...
  const review = trpc.admin.reviewMilestone.useMutation({
    onSuccess: () => {
      toast("Review saved — score awarded where due.");
      invalidate();
      utils.admin.enrolmentDetail.invalidate();
    },
    onError: e => toast(e.message),
  });

  const detail = trpc.admin.enrolmentDetail.useQuery(
    { id: enrolId! },
    { enabled: enrolId !== null, retry: false }
  );
  ...
  function onCreate(e: FormEvent<HTMLFormElement>) {
    ...
    createCohort.mutate({
      ...
      startsAt: f.get("startsAt")
        ? new Date(String(f.get("startsAt")))
        : undefined,
    });
  }
                ...
                <tbody>
                  {c.enrolments.map(({ en, member, userName }) => (
                    <tr key={en.id}>
                      ...
                      <td className="eh-sm eh-muted" data-label="Enrolled">
                        {fmtDate(en.createdAt)}
                      </td>
                      <td>
                        <button
                          className="eh-btn ghost sm"
                          onClick={() => setEnrolId(en.id)}
                        >
                          Review →
                        </button>
                      </td>
          ...
          <form onSubmit={onCreate}>
            <Field label="Name">
              <input
                ...
                name="name"
                required
                placeholder="FRP Cohort 4 — Spring 2027"
              />
            </Field>
              ...
              <div className="eh-between eh-mb">
                <div>
                  ...
                  <span className="eh-muted eh-sm">
                    {" "}
                    · {detail.data.member.company} · {detail.data.cohort.name}
                  </span>
                </div>
                    ...
                    {(
                      ...
                      <div key={k}>
                        ...
                        >
                          ...
                          <span className="eh-num eh-sm">
                            {detail.data!.assessment![k]}/5
                          </span>
                        </div>
                ...
                {detail.data.milestones.map(m => (
                  ...
                  >
                    <div style={{ flex: 1 }}>
                      ...
                      {m.note && (
                        <div className="d" style={{ maxWidth: "46ch" }}>
                          {m.note}
                        </div>
                      )}
                    </div>
