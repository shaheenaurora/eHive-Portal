...
export default function AdminDashboard() {
  ...
  return (
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      <PageHead eyebrow="Admin dashboard" title="The circle at a glance"
      ...
      <div className="eh-grid g4">
        <Stat k="Active members" v={s.activeMembers} n="Across all four tiers" />
        <Stat k="Pending applications" v={s.pendingApplications} gold n="Received → interview stages" />
        <Stat k="Average Hive Score" v={s.avgScore} n="Active members" />
        <Stat k="Upcoming events" v={s.upcomingEvents} n={`${s.totalLeads} website leads captured`} />
      </div>
            ...
            {(["zenith", "vanguard", "ascent", "horizon"] as const).map((t) => (
              <div className="row" key={t} style={{ display: "block" }}>
                <div className="eh-between" style={{ marginBottom: ".3rem" }}>
                  ...
                  <span className="eh-num eh-sm eh-strong">{tierMap.get(t) ?? 0}</span>
                </div>
                <Bar pct={((tierMap.get(t) ?? 0) / Math.max(1, s.activeMembers)) * 100} />
              </div>
            ))}
          </div>
        </div>
            ...
            {BANDS.map((b) => (
              <div className="row" key={b} style={{ display: "block" }}>
                <div className="eh-between" style={{ marginBottom: ".3rem" }}>
                  ...
                  <span className="eh-num eh-sm eh-strong">{distMap.get(b) ?? 0}</span>
                </div>
                <Bar pct={((distMap.get(b) ?? 0) / maxBand) * 100} green={b === "80+"} />
              </div>
            ))}
          </div>
        </div>
