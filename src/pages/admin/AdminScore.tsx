...
export default function AdminScore() {
  ...
  const recompute = trpc.admin.recomputeAll.useMutation({
    onSuccess: (r) => { toast(`Recomputed ${r.recomputed} members.`); utils.admin.scoreConfig.invalidate(); utils.admin.members.invalidate(); },
    onError: (e) => toast(e.message),
  });
  ...
  return (
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      <PageHead eyebrow="Hive Score engine" title="Weights and recalculation"
          ...
          {q.data && (
            <>
              <div className="eh-list">
                {SCORE_FACTORS.map((f) => (
                  <div className="row" key={f}>
                    ...
                    <input className="eh-input eh-num" type="number" min={0} max={100} style={{ maxWidth: 90, textAlign: "right" }}
                           value={weights[f] ?? 0}
              ...
              <div className="eh-between">
                <Pill color={total === 100 ? "green" : "red"}>Total: {total}/100</Pill>
                <div className="eh-row">
                  <button className="eh-btn" disabled={save.isPending || total !== 100}
                          onClick={() => save.mutate(SCORE_FACTORS.map((f) => ({ factor: f, weight: weights[f] ?? 0 })))}>
                    Save weights
                  </button>
                  <button className="eh-btn gold" disabled={recompute.isPending}
            ...
            {q.data?.top.map(({ member, userName }, i) => (
              <div className="row" key={member.id}>
                <div className="eh-row" style={{ flexWrap: "nowrap" }}>
                  <span className="eh-num eh-muted" style={{ width: 20 }}>{i + 1}</span>
                  <div>
