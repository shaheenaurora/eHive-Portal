...
type CaseRow = {
  id: number; category: string; severity: ConductSeverity; status: ConductStatus;
  summary: string; detail: string | null; resolution: string | null;
  reporterMemberId: number | null; subjectMemberId: number | null;
  reporterName: string; subjectName: string | null; createdAt: string | Date;
};

const SEV_COLOR: Record<ConductSeverity, "grey" | "blue" | "gold" | "red"> = {
  low: "grey", moderate: "blue", high: "gold", safeguarding: "red",
};
const STATUS_COLOR: Record<ConductStatus, "grey" | "blue" | "gold" | "green" | "red"> = {
  open: "red", reviewing: "gold", actioned: "green", escalated: "red", closed: "grey",
};
    ...
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      ...
      <div className="eh-tabs">
        ...
        {CONDUCT_STATUSES.map((s) => (
          <button key={s} className={status === s ? "on" : ""} onClick={() => setStatus(s)}>{CONDUCT_STATUS_LABEL[s]}</button>
        ))}
      </div>

      {q.isLoading && <Spinner />}
      {q.isError && <div className="eh-card"><Empty big="Couldn't load cases." p="You may not have the Conduct & Safeguarding capability."><button className="eh-btn ghost" onClick={() => q.refetch()}>Retry</button></Empty></div>}
      {q.data && q.data.length === 0 && <div className="eh-card"><Empty big="No cases here." p="Confidential reports from members appear here the moment they're raised." /></div>}

      {q.data && q.data.length > 0 && (
        <div className="eh-card" style={{ padding: ".4rem 1.25rem" }}>
          <table className="eh-table stack">
            <thead><tr><th>Summary</th><th>Category</th><th>Severity</th><th>Status</th><th>Subject</th><th>Raised</th><th></th></tr></thead>
            <tbody>
              {(q.data as CaseRow[]).map((c) => (
                <tr key={c.id} className="click" onClick={() => setSel(c)}>
                  <td><b>{c.summary}</b></td>
                  <td data-label="Category" className="eh-sm">{c.category}</td>
                  <td data-label="Severity"><Pill color={SEV_COLOR[c.severity]}>{CONDUCT_SEVERITY_LABEL[c.severity]}</Pill></td>
                  <td data-label="Status"><Pill color={STATUS_COLOR[c.status]}>{CONDUCT_STATUS_LABEL[c.status]}</Pill></td>
                  <td data-label="Subject" className="eh-sm">{c.subjectName ?? "—"}</td>
                  <td data-label="Raised" className="eh-sm eh-muted">{fmtDateTime(c.createdAt)}</td>
      ...
      {sel && <CaseDetail c={sel} onClose={() => setSel(null)} onSaved={() => q.refetch()} />}
    </EhShell>
  );
}

function CaseDetail({ c, onClose, onSaved }: { c: CaseRow; onClose: () => void; onSaved: () => void }) {
  ...
  const update = trpc.conduct.updateCase.useMutation({
    onSuccess: () => { toast("Case updated."); onSaved(); },
    onError: (e) => toast(e.message),
  });
  const act = trpc.conduct.actionMember.useMutation({
    onSuccess: (r) => { toast(`Member set to ${r.lifecycleState}.`); onSaved(); onClose(); },
    onError: (e) => toast(e.message),
  });
  ...
  return (
    <Modal title={`Case #${c.id}`} onClose={onClose} wide>
      ...
      <div className="eh-list eh-mb">
        <div className="row"><span className="d">Summary</span><span className="t">{c.summary}</span></div>
        <div className="row"><span className="d">Category</span><span className="t">{c.category}</span></div>
        <div className="row"><span className="d">Reported by</span><span className="t">{c.reporterName}</span></div>
        <div className="row"><span className="d">Subject</span><span className="t">{c.subjectName ?? "—"}</span></div>
        {c.detail && <div className="row" style={{ alignItems: "flex-start" }}><span className="d">Detail</span><span className="t eh-sm" style={{ textAlign: "right", whiteSpace: "pre-wrap" }}>{c.detail}</span></div>}
      </div>

      <div className="eh-eyebrow">Handle the case</div>
      <div className="eh-grid g2 eh-mb" style={{ marginTop: ".4rem" }}>
        <Field label="Status">
          <select className="eh-select" value={status} onChange={(e) => setStatus(e.target.value as ConductStatus)}>
            {CONDUCT_STATUSES.map((s) => <option key={s} value={s}>{CONDUCT_STATUS_LABEL[s]}</option>)}
          </select>
        </Field>
      ...
      <Field label="Resolution / process notes">
        <textarea className="eh-textarea" value={resolution} onChange={(e) => setResolution(e.target.value)}
                  placeholder="What was decided, who was consulted, the outcome…" />
      </Field>
      <button className="eh-btn gold" disabled={update.isPending}
              onClick={() => update.mutate({ id: c.id, status, severity, resolution })}>
        {update.isPending ? "Saving…" : "Save case"}
      </button>

      {c.subjectMemberId && (
        <>
          <div className="eh-eyebrow eh-mt">Action the member <span className="eh-muted">· {c.subjectName}</span></div>
          <p className="eh-sm eh-muted" style={{ margin: ".3rem 0 .6rem" }}>
            Changes the member's lifecycle state and notifies them confidentially. Use with care — every action is
