...
export default function AdminApplications() {
  ...
  const q = trpc.admin.applications.useQuery(
  ...
  const setStatusMut = trpc.admin.setApplicationStatus.useMutation({
    onSuccess: r => {
      toast(
        r.memberId
          ? `Approved — membership #${r.memberId} created.`
          : "Status updated."
      );
            ...
            <tbody>
              {rows.map(a => (
                ...
                >
                  ...
                  <td className="eh-sm" data-label="Stage">
                    {a.stage ?? "—"}
                  </td>
                  <td data-label="Tier">
                    <TierPill tier={a.tierRequested} />
                  </td>
      ...
      {sel && (
        ...
        >
          <div className="eh-grid g2" style={{ marginBottom: "1rem" }}>
            <div className="eh-list">
              <div className="row">
                ...
                <span className="t eh-sm">{sel.email}</span>
              </div>
              <div className="row">
                <span className="d">Company</span>
                <span className="t eh-sm">{sel.company ?? "—"}</span>
              </div>
              <div className="row">
                <span className="d">Stage</span>
                <span className="t eh-sm">{sel.stage ?? "—"}</span>
              </div>
              <div className="row">
                <span className="d">Revenue</span>
                <span className="t eh-sm">{sel.revenue ?? "—"}</span>
              </div>
            </div>
            <div className="eh-list">
              <div className="row">
                <span className="d">Applied</span>
                <span className="t eh-sm">{fmtDate(sel.createdAt)}</span>
              </div>
              <div className="row">
                <span className="d">Tier requested</span>
                <TierPill tier={sel.tierRequested} />
              </div>
              <div className="row">
          ...
          <div className="eh-grid g2">
            ...
            <Field
              label={
                hasChapters
                  ? "Admit into chapter"
                  : "Admit into chapter (create a chapter first)"
              }
            >
              <select
                className="eh-select"
              ...
              >
                <option value="">
                  {hasChapters ? "Select a chapter…" : "No chapters yet"}
                </option>
                {(chapters.data ?? []).map(c => (
          ...
          {sel.status !== "approved" && hasChapters && !chapter && (
            <p
              className="eh-sm"
              style={{ color: "var(--eh-gold)", margin: ".2rem 0 .6rem" }}
            >
            ...
            {sel.status !== "approved" && (
              <>
                ...
                <button
                  ...
                  disabled={setStatusMut.isPending || (hasChapters && !chapter)}
                  onClick={() =>
                    setStatusMut.mutate({
                      id: sel.id,
                      status: "approved",
