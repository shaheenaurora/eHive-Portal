...
export default function AdminEngagement() {
  ...
  const rules = trpc.adminEngage.pointRules.useQuery(undefined, {
    retry: false,
  });
  const config = trpc.adminEngage.engagementConfig.useQuery(undefined, {
    retry: false,
  });
  const board = trpc.adminEngage.dormancyBoard.useQuery(undefined, {
    retry: false,
  });
  ...
  const [pauseFor, setPauseFor] = useState<{ id: number; name: string } | null>(
    null
  );
  ...
  const setRule = trpc.adminEngage.setPointRule.useMutation({
    onSuccess: () => {
      toast("Point rule updated — applies from the next event.");
      setEditRule(null);
      refresh();
    },
  ...
  const setCfg = trpc.adminEngage.setEngagementConfig.useMutation({
    onSuccess: () => {
      toast("Engagement Standard saved.");
      setEditCfg(null);
      refresh();
    },
    onError: e => toast(e.message),
  ...
  const evaluate = trpc.adminEngage.runDormancyEvaluation.useMutation({
    onSuccess: r => {
      toast(
        `Evaluation complete — ${r.evaluated} members reviewed, ${r.transitions} stage changes.`
      );
      refresh();
    },
    onError: e => toast(e.message),
  ...
  const override = trpc.adminEngage.setDormancyOverride.useMutation({
    onSuccess: () => {
      toast("Stage overridden and logged.");
      setOverrideFor(null);
      refresh();
    },
    onError: e => toast(e.message),
    ...
    <EhShell groups={ADMIN_NAV} brandSub="Admin Portal" roleRequired="admin">
      ...
      <div className="eh-grid g2" style={{ alignItems: "start" }}>
        ...
        <div className="eh-card">
          ...
          <p className="eh-muted eh-sm">
            The minimum a member owes the Circle. Evaluated quarterly against
            the Dormancy Ladder.
          </p>
          <div className="eh-list">
          ...
          <div className="eh-between">
            <div>
              ...
              <p className="eh-muted eh-sm" style={{ margin: 0 }}>
                Runs the ladder for every active member and notifies
                transitions.
              </p>
            </div>
            <button
      ...
      <h2 className="eh-h2" style={{ margin: "1.75rem 0 .75rem" }}>
        Dormancy board
      </h2>
      <div className="eh-card">
          ...
          <table className="eh-table stack">
            ...
            <tbody>
              {(board.data?.rows ?? []).map(r => {
                const stage = (r.member.dormancyStage ??
                ...
                return (
                  <tr key={r.member.id}>
                    ...
                    <td data-label="Tier">
                      {TIER_LABEL[r.member.tier as Tier]}
                    </td>
                    <td data-label="Stage">
                      <Pill color={STAGE_COLOR[stage] ?? "grey"}>
                        {DORMANCY_LABEL[stage]}
                      </Pill>
                    </td>
                    <td className="eh-num" data-label="Pause">
                      {r.member.exceptionPause > 0
                        ? `${r.member.exceptionPause}q`
                        : "—"}
                    </td>
                    <td className="eh-muted" data-label="Note">
                      {r.member.dormancyNote ?? "—"}
                    </td>
                ...
                <div className="row" key={l.id}>
                  ...
                  <span className="t eh-sm" style={{ flex: 1 }}>
                    ...
                    {DORMANCY_LABEL[l.fromStage as DormancyStage] ??
                      l.fromStage}{" "}
                    → {DORMANCY_LABEL[l.toStage as DormancyStage] ?? l.toStage}
                    {l.reason ? ` — ${l.reason}` : ""}
                  </span>
                  <Pill color={l.actor === "system" ? "grey" : "purple"}>
                    {l.actor}
                  </Pill>
                </div>
              ))}
      ...
      {editRule && (
        <Modal
          title={`Point rule — ${editRule.label}`}
        ...
        >
          ...
          <button
            ...
            onClick={() =>
              setRule.mutate({ key: editRule.key as never, points })
            }
          >
      ...
      {editCfg && (
        <Modal
          title={`Engagement Standard — ${TIER_LABEL[editCfg as Tier]}`}
          onClose={() => setEditCfg(null)}
        >
          <CfgForm
            tier={editCfg as Tier}
            existing={cfgByTier.get(editCfg as Tier)}
      ...
      {overrideFor && (
        <Modal
          title={`Override stage — ${overrideFor.name}`}
          onClose={() => setOverrideFor(null)}
        >
          <OverrideForm
            pending={override.isPending}
            onSubmit={(stage, note) =>
              override.mutate({ memberId: overrideFor.id, stage, note })
            }
          />
        </Modal>
      )}
      ...
      {pauseFor && (
        <Modal
          title={`Exception pause — ${pauseFor.name}`}
          onClose={() => setPauseFor(null)}
        >
          <PauseForm
            pending={pause.isPending}
            onSubmit={(quarters, note) =>
              pause.mutate({ memberId: pauseFor.id, quarters, note })
            }
          />
        </Modal>
      )}
...
function CfgForm(props: {
  ...
  existing?: {
    ...
    sessionsOffered: number | null;
    oneToOnesPerQuarter: number | null;
    giveBackPerYear: number | null;
  };
  pending: boolean;
  onSubmit: (v: {
    sessionsRequired: number | null;
  ...
  const [sr, setSr] = useState(
    props.existing?.sessionsRequired?.toString() ?? ""
  );
  const [so, setSo] = useState(
    props.existing?.sessionsOffered?.toString() ?? ""
  );
  const [ot, setOt] = useState(
    props.existing?.oneToOnesPerQuarter?.toString() ?? ""
  );
  const [gb, setGb] = useState(
    props.existing?.giveBackPerYear?.toString() ?? ""
  );
  const num = (s: string) =>
    s.trim() === "" ? null : Math.max(0, Number(s) || 0);
  return (
    <>
      <div className="eh-grid g2">
        <Field label="Sessions required / year">
          <input
            className="eh-input"
            type="number"
            min={0}
        ...
        <Field label="Sessions offered / year">
          <input
            className="eh-input"
            type="number"
            min={0}
        ...
        <Field label="1-2-1s per quarter">
          <input
            className="eh-input"
            type="number"
            min={0}
        ...
        <Field label="Give-Back per year">
          <input
            className="eh-input"
            type="number"
            min={0}
      ...
      <button
        ...
        onClick={() =>
          props.onSubmit({
            sessionsRequired: num(sr),
            sessionsOffered: num(so),
            oneToOnesPerQuarter: num(ot),
            giveBackPerYear: num(gb),
          })
        }
      >
...
function OverrideForm(props: {
  pending: boolean;
  onSubmit: (stage: DormancyStage, note: string) => void;
}) {
    ...
    <>
      <Field label="Set stage">
        <select
          className="eh-select"
          value={stage}
          onChange={e => setStage(e.target.value as DormancyStage)}
        >
          {(Object.keys(DORMANCY_LABEL) as DormancyStage[]).map(s => (
      ...
      <button
        className="eh-btn gold"
        style={{ width: "100%" }}
        disabled={props.pending || note.trim().length < 3}
...
function PauseForm(props: {
  pending: boolean;
  onSubmit: (quarters: number, note?: string) => void;
}) {
    ...
    <>
      <Field label="Pause evaluation for (quarters; 0 clears)">
        <input
          ...
          value={quarters}
          onChange={e =>
            setQuarters(Math.max(0, Math.min(4, Number(e.target.value) || 0)))
          }
        />
      </Field>
