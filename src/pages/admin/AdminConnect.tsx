...
export default function AdminConnect() {
  ...
  const buddy = trpc.adminEngage.buddyBoard.useQuery(undefined, {
    retry: false,
  });
  const refs = trpc.adminEngage.referralsAdmin.useQuery(undefined, {
    retry: false,
  });
  const deals = trpc.adminEngage.dealsAdmin.useQuery(undefined, {
    retry: false,
  });
  const oneToOnes = trpc.adminEngage.oneToOnesAdmin.useQuery(undefined, {
    retry: false,
  });
  ...
  const pair = trpc.adminEngage.pairBuddy.useMutation({
    onSuccess: () => {
      toast("Buddy paired — both members notified.");
      setPairOpen(false);
      setNewId(0);
      setBuddyId(0);
      refresh();
      ...
      {tab === "buddies" && (
        <>
          {(buddy.data?.unpaired.length ?? 0) > 0 && (
            <div className="eh-card eh-mb" style={{ borderColor: "#b8862e" }}>
              ...
              <p className="eh-muted eh-sm">
                BRD rule: every new member is paired within 5 days of joining.
              </p>
          ...
          <div className="eh-card">
            ...
            {buddy.data && buddy.data.pairs.length === 0 && (
              <Empty big="No pairs yet." />
            )}
            <div className="eh-list">
        ...
        <div className="eh-card">
          ...
          {refs.data && refs.data.length === 0 && (
            <Empty
              ...
              p="Member-submitted referrals land here for review."
            />
          )}
          <div className="eh-list">
            {(refs.data ?? []).map(r => (
              <div className="row" key={r.id}>
                <div style={{ flex: 1 }}>
                  <div className="t">
                    {r.prospectName}{" "}
                    <span className="eh-muted eh-sm">via {r.memberName}</span>
                  </div>
                  <div className="d">
                    {fmtDate(r.createdAt)}
                    {r.prospectContact ? ` · ${r.prospectContact}` : ""}
                    {r.note ? ` — ${r.note}` : ""}
                  </div>
                </div>
                {r.status === "submitted" && (
                  <>
                    <button
                      ...
                      onClick={() =>
                        setRefStatus.mutate({ id: r.id, status: "converted" })
                      }
                    >
                      Mark converted
                    </button>
                    <button
                      ...
                      disabled={setRefStatus.isPending}
                      onClick={() =>
                        setRefStatus.mutate({ id: r.id, status: "rejected" })
                      }
                    >
                      Not a fit
                    </button>
        ...
        <>
          <div className="eh-between eh-mb">
            <p className="eh-muted eh-sm" style={{ margin: 0 }}>
              Staff posts and moderation. Member posts respect the give-to-get
              rule automatically.
            </p>
            <button className="eh-btn gold" onClick={() => setDealOpen(true)}>
              Post as staff →
            </button>
          </div>
              ...
              <div className="eh-card" key={d.id}>
                <div className="eh-between">
                  ...
                  <span className="eh-muted eh-sm">
                    {fmtDate(d.createdAt)} ·{" "}
                    {d.postedBy ? `member #${d.postedBy}` : "staff"}
                  </span>
                </div>
                ...
                <button
                  ...
                  disabled={delDeal.isPending}
                  onClick={async () => {
                    if (
                      await confirmDialog({
                        title: "Remove deal?",
                        body: "This deal will be removed from the Deal Flow board.",
                        danger: true,
                        confirmLabel: "Remove",
                      })
                    ) {
                      delDeal.mutate({ id: d.id });
                    }
                  }}
                >
                  Remove
      ...
      {tab === "121" && (
        <div className="eh-card">
          ...
          {oneToOnes.data && oneToOnes.data.length === 0 && (
            <Empty big="No 1-2-1s logged yet." />
          )}
          <div className="eh-list">
            {(oneToOnes.data ?? []).map(r => (
              <div className="row" key={r.id}>
                <div style={{ flex: 1 }}>
                  <div className="t">
                    {r.kind === "mentoring" ? "Mentoring" : "1-2-1"} — {r.aName}{" "}
                    ⇄ {r.bName}
                  </div>
                  <div className="d">
                    {fmtDate(r.createdAt)}
        ...
        <Modal title="Pair a buddy" onClose={() => setPairOpen(false)}>
          ...
          <Field label="Buddy (active member)">
            <select
              className="eh-select"
              value={buddyId}
              onChange={e => setBuddyId(Number(e.target.value))}
            >
              <option value={0}>Pick…</option>
              {(buddy.data?.candidates ?? []).map(c => (
                <option key={c.id} value={c.id}>
          ...
          <button
            ...
            style={{ width: "100%" }}
            disabled={pair.isPending || !newId || !buddyId || newId === buddyId}
            onClick={() =>
              pair.mutate({ newMemberId: newId, buddyMemberId: buddyId })
            }
          >
        ...
        <Modal title="Post a deal (staff)" onClose={() => setDealOpen(false)}>
          <StaffDealForm
            ...
            onSubmit={(title, description, tierGate) =>
              saveDeal.mutate({ title, description, tierGate })
            }
          />
        </Modal>
      )}
  ...
  return (
    <>
      ...
      <Field label="Visible from tier">
        ...
        >
          {(["horizon", "ascent", "vanguard", "zenith"] as const).map(t => (
