...
import { EhShell, ADMIN_NAV, Pill, StatusPill, TierPill, Empty, Spinner, Modal, Field, toast } from "@/components/eh";
...
export default function AdminPodDetail() {
  ...
  const [notesFor, setNotesFor] = useState<{ sessionId: number; summary: string } | null>(null);
  ...
  const addToPod = trpc.admin.addToPod.useMutation({
    onSuccess: () => { toast("Added to roster."); invalidate(); setAddMember(false); },
    onError: (e) => toast(e.message),
  });
  const removeFromPod = trpc.admin.removeFromPod.useMutation({
    onSuccess: () => { toast("Removed from roster."); invalidate(); },
    onError: (e) => toast(e.message),
  });
  const createSession = trpc.admin.createSession.useMutation({
    onSuccess: () => { toast("Session scheduled."); invalidate(); setAddSession(false); },
    onError: (e) => toast(e.message),
  });
  ...
  const saveNotes = trpc.admin.saveSessionNotes.useMutation({
    onSuccess: () => { toast("Notes published to the pod."); invalidate(); setNotesFor(null); },
    onError: (e) => toast(e.message),
  });
  const markAtt = trpc.admin.markAttendance.useMutation({
    onSuccess: () => { toast("Attendance marked — score updated."); invalidate(); },
    onError: (e) => toast(e.message),
  });
  const assignTask = trpc.admin.assignActionItem.useMutation({
    onSuccess: () => { toast("Commitment assigned."); invalidate(); setTaskFor(false); },
    onError: (e) => toast(e.message),
  });
  ...
  return (
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      <div className="eh-page-head">
        <div>
          <div className="eh-eyebrow"><Link to="/admin/pods" style={{ color: "inherit" }}>← Pods</Link></div>
        ...
        <div className="eh-row">
          ...
          <button className="eh-btn sm" onClick={() => setAddMember(true)}>+ Member</button>
        </div>
      </div>

      <div className="eh-grid g3" style={{ alignItems: "start" }}>
        <div className="eh-card">
          <h3>Roster ({roster.length}/{pod.capacity})</h3>
          {roster.length === 0 && <Empty big="Empty room." p="Add the first member." />}
            ...
            {roster.map(({ member, userName, userEmail, pm }) => (
              <div className="row" key={pm.id}>
                ...
                <div className="eh-row">
                  ...
                  <button className="eh-btn ghost sm" onClick={() => removeFromPod.mutate({ podId, memberId: member.id })}>✕</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="eh-span2" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div className="eh-card">
            ...
            <div className="eh-list">
              {sessions.map((s) => {
                ...
                return (
                  <div className="row" key={s.id} style={{ alignItems: "flex-start", display: "block" }}>
                    <div className="eh-between">
                      <div>
                        <div className="t">{s.topic ?? "Pod session"}</div>
                        <div className="d">{fmtDay(s.startsAt)} · {fmtDateTime(s.startsAt).split("·")[1]} · {s.durationMin} min · {att.length} attendance marks</div>
                      </div>
                      <div className="eh-row">
                        <StatusPill status={s.status} />
                        <button className="eh-btn ghost sm" onClick={() => setAttSession(attSession === s.id ? null : s.id)}>
                          Attendance
                        </button>
                        <button className="eh-btn ghost sm" onClick={() => setNotesFor({ sessionId: s.id, summary: note?.summary ?? "" })}>
                          Notes
                        </button>
                        {s.status === "scheduled" && (
                          <button className="eh-btn sm" onClick={() => setSessionStatus.mutate({ id: s.id, status: "done" })}>
                            Mark done
                          </button>
                        )}
                      </div>
                    ...
                    {attSession === s.id && (
                      <div className="eh-card" style={{ background: "var(--eh-paper)", marginTop: ".7rem", padding: ".8rem 1rem" }}>
                        <div className="eh-list">
                          {roster.map(({ member, userName }) => {
                            ...
                            return (
                              <div className="row" key={member.id}>
                                ...
                                <div className="eh-row">
                                  {(["attended", "excused", "absent"] as const).map((st) => (
                                    <button key={st}
                                      className={"eh-btn sm" + (cur?.status === st ? (st === "attended" ? " gold" : "") : " ghost")}
                                      onClick={() => markAtt.mutate({ sessionId: s.id, memberId: member.id, status: st })}>
                                      {st}
                                    </button>
                                  ))}
                                </div>
                    ...
                    {note?.summary && <p className="eh-sm eh-muted" style={{ margin: ".5rem 0 0", whiteSpace: "pre-line" }}>{note.summary}</p>}
                  </div>
                );
              })}
            </div>
          ...
          <div className="eh-card">
            <div className="eh-between">
              ...
              <button className="eh-btn sm" onClick={() => setTaskFor(true)}>+ Assign</button>
            </div>
            ...
            <div className="eh-list">
              {actionItems.map(({ ai, userName }) => (
                <div className="row" key={ai.id}>
                  <div>
                    <div className="t" style={ai.status === "done" ? { textDecoration: "line-through", opacity: .6 } : undefined}>{ai.text}</div>
                    <div className="d">{userName ?? "—"} · due {ai.dueAt ? fmtDay(ai.dueAt) : "—"}</div>
                  </div>
      ...
      {addMember && (
        <Modal title="Add member to roster" onClose={() => setAddMember(false)}>
          <div className="eh-list" style={{ maxHeight: 380, overflowY: "auto" }}>
            {notInPod.length === 0 && <Empty big="Everyone is already in." />}
            {notInPod.map((m) => (
              <div className="row" key={m.id}>
                <div>
                  <div className="t">{m.userName ?? "—"}</div>
                  <div className="d">{m.company ?? m.userEmail ?? ""} · {m.tier}</div>
                </div>
                <button className="eh-btn gold sm" disabled={addToPod.isPending}
                        onClick={() => addToPod.mutate({ podId, memberId: m.id })}>Add</button>
              </div>
            ))}
          </div>
        </Modal>
      ...
      {addSession && (
        <Modal title="Schedule a session" onClose={() => setAddSession(false)}>
          <form onSubmit={onAddSession}>
            <Field label="Topic">
              <input className="eh-input" name="topic" placeholder="Q4 planning" />
            </Field>
            <div className="eh-grid g2">
              ...
              <Field label="Duration (min)">
                <input className="eh-input" name="durationMin" type="number" min={15} max={480} defaultValue={90} />
              </Field>
            </div>
            ...
            <button className="eh-btn gold" type="submit" disabled={createSession.isPending}>Schedule →</button>
          </form>
        </Modal>
      )}

      {notesFor && (
        <Modal title="Session notes (visible to the pod)" onClose={() => setNotesFor(null)}>
          <Field label="Summary">
            <textarea className="eh-textarea" style={{ minHeight: 160 }} value={notesFor.summary}
                      onChange={(e) => setNotesFor({ ...notesFor, summary: e.target.value })}
                      placeholder="What was decided, who committed to what." />
          </Field>
          <button className="eh-btn gold" disabled={saveNotes.isPending}
                  onClick={() => saveNotes.mutate({ sessionId: notesFor.sessionId, summary: notesFor.summary })}>
            Publish notes →
          </button>
        </Modal>
      )}
          ...
          <form onSubmit={onAssign}>
            <Field label="Member">
              <select className="eh-select" name="memberId" required>
                {roster.map(({ member, userName }) => (
                  <option key={member.id} value={member.id}>{userName}</option>
                ))}
              </select>
            </Field>
            <Field label="The commitment">
              <textarea className="eh-textarea" name="text" required minLength={2} style={{ minHeight: 70 }}
            ...
            <div className="eh-grid g2">
              ...
              <Field label="From session (optional)">
                <select className="eh-select" name="sessionId" defaultValue="">
                  <option value="">—</option>
                  {sessions.map((s) => <option key={s.id} value={s.id}>{s.topic ?? fmtDay(s.startsAt)}</option>)}
                </select>
              </Field>
            </div>
            <button className="eh-btn gold" type="submit" disabled={assignTask.isPending}>Assign →</button>
          </form>
        </Modal>
      )}
    </EhShell>
