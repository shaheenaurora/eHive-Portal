...
export default function AdminGovernance() {
  ...
  const publishMinutes = trpc.admin.publishMinutes.useMutation({
    onSuccess: () => {
      toast("Minutes published to all members.");
      invalidate();
      setMinutesFor(null);
    },
    onError: e => toast(e.message),
  ...
  function onSeat(e: FormEvent<HTMLFormElement>) {
    ...
    assignSeat.mutate({
      ...
      termStart: f.get("termStart")
        ? new Date(String(f.get("termStart")))
        : undefined,
      termEnd: f.get("termEnd")
        ? new Date(String(f.get("termEnd")))
        : undefined,
    });
  }
  ...
  return (
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      <PageHead
        eyebrow="Governance"
        title="Bodies, seats, minutes, policies"
        sub="Everything published here is visible to every member immediately."
        actions={
          <button className="eh-btn gold" onClick={() => setBodyFor(true)}>
            + New body
          </button>
        }
      />
          ...
          {q.data?.bodies.map(b => (
            <div className="eh-card" key={b.id}>
              <div className="eh-between">
                ...
                <div className="eh-row">
                  <button
                    ...
                    onClick={() => setSeatFor(b.id)}
                  >
                    + Seat
                  </button>
                  <button
                    className="eh-btn ghost sm"
                    onClick={() => setMinutesFor(b.id)}
                  >
                    + Minutes
                  </button>
                </div>
              ...
              <div className="eh-list">
                  ...
                  <div className="row" key={role.id}>
                    <div className="eh-row" style={{ flexWrap: "nowrap" }}>
                      ...
                      <div>
                        ...
                        <div className="d">
                          {fmtDate(role.termStart)} → {fmtDate(role.termEnd)}
                        </div>
                      </div>
                    </div>
                    <div className="eh-row">
                      ...
                      <button
                        ...
                        onClick={async () => {
                          if (
                            await confirmDialog({
                              title: "Remove seat?",
                              body: `This will remove ${userName ?? "the member"} from the ${role.seat} seat.`,
                              danger: true,
                              confirmLabel: "Remove",
                            })
                          ) {
                            removeSeat.mutate({ id: role.id });
                          }
                        }}
                      >
                        ✕
                ...
                {b.roles.length === 0 && (
                  <p className="eh-sm eh-muted">No seats assigned.</p>
                )}
              </div>
              {b.minutes.length > 0 && (
                <>
        ...
        <div className="eh-card">
          <div className="eh-between">
            ...
            <button
              ...
              onClick={() => setPolicyFor({ title: "", body: "", version: 1 })}
            >
              + New policy
            </button>
          </div>
            ...
            {q.data?.policies.map(p => (
              <div className="row" key={p.id}>
                <div>
                  ...
                  <div className="d">
                    v{p.version} · {p.ackCount} acknowledgments
                  </div>
                </div>
                <button
                  className="eh-btn ghost sm"
          ...
          <form onSubmit={onSeat}>
            ...
            <div className="eh-grid g2">
              <Field label="Term start">
                <input className="eh-input" name="termStart" type="date" />
              </Field>
              <Field label="Term end">
                <input className="eh-input" name="termEnd" type="date" />
              </Field>
            </div>
            <button
              className="eh-btn gold"
          ...
          <form onSubmit={onMinutes}>
            <Field label="Title">
              <input
                ...
                name="title"
                required
                placeholder="Council minutes — August 2026"
              />
            </Field>
            <Field label="Date">
              <input className="eh-input" name="date" type="date" />
            </Field>
            <Field label="Minutes">
              <textarea
            ...
            <button
              className="eh-btn gold"
              type="submit"
              disabled={publishMinutes.isPending}
            >
              Publish →
            </button>
          </form>
      ...
      {policyFor && (
        <Modal
          title={
            policyFor.id ? `New version of “${policyFor.title}”` : "New policy"
          }
        ...
        >
          <form
            onSubmit={e => {
              ...
              savePolicy.mutate({
                id: policyFor.id,
                title: policyFor.title,
                body: policyFor.body,
                version: policyFor.version,
              });
            }}
          >
            <Field label="Title">
              <input
                className="eh-input"
                value={policyFor.title}
                required
                onChange={e =>
                  setPolicyFor({ ...policyFor, title: e.target.value })
                }
              />
            </Field>
            <Field
              label={`Version ${policyFor.version} text (members must re-acknowledge)`}
            >
              <textarea
                className="eh-textarea"
                style={{ minHeight: 180 }}
                value={policyFor.body}
                required
                onChange={e =>
                  setPolicyFor({ ...policyFor, body: e.target.value })
                }
              />
            </Field>
            <button
