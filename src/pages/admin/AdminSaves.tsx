...
type SaveCase = {
  id: number; memberId: number; memberName: string | null; tier: string | null;
  chapterName: string | null; status: SaveCaseStatus; reason: string;
  ...
  openedAt: string | Date; closedAt: string | Date | null; daysOpen: number;
};
...
function urgency(daysOpen: number): { color: string; label: string } {
  if (daysOpen >= 7) return { color: "var(--eh-red, #b23a2e)", label: `${daysOpen}d open` };
  if (daysOpen >= 3) return { color: "#b8862e", label: `${daysOpen}d open` };
  return { color: "var(--eh-good, #2e7d5b)", label: daysOpen === 0 ? "New today" : `${daysOpen}d open` };
}

export default function AdminSaves() {
  const [filter, setFilter] = useState<Filter>("open");
  const q = trpc.adminEngage.savesList.useQuery({ status: filter }, { retry: false });
  const summary = trpc.adminEngage.savesSummary.useQuery(undefined, { retry: false });
  const me = trpc.auth.me.useQuery(undefined, { retry: false });
  const utils = trpc.useUtils();
  const refresh = () => { utils.adminEngage.savesList.invalidate(); utils.adminEngage.savesSummary.invalidate(); };
    ...
    <EhShell groups={ADMIN_NAV} brandSub="Admin">
      ...
      {s && (
        <div className="eh-grid g4 eh-mb">
          <MetricTile k="Active saves" v={active} n={`${s.open} untouched · ${s.working} in progress`} />
          <MetricTile k="Need attention" v={s.overdue} n="Open more than 7 days"
            accent={s.overdue > 0 ? "var(--eh-red, #b23a2e)" : undefined} />
          <MetricTile k="Saved" v={s.saved} n={`${s.lost} lost`} accent="var(--eh-good, #2e7d5b)" />
          <MetricTile k="Save rate" v={s.saveRate == null ? "—" : `${s.saveRate}%`} n="Of all closed cases" />
        </div>
      )}
      ...
      <div className="eh-between eh-mb" style={{ flexWrap: "wrap", gap: ".6rem" }}>
        ...
        {rows.length > 0 && <span className="eh-muted eh-sm">{rows.length} case{rows.length === 1 ? "" : "s"} shown</span>}
      </div>
      ...
      <div style={{ display: "grid", gap: ".7rem" }}>
        {rows.map((c) => (
          <CaseCard key={c.id} c={c} me={me.data?.id ?? null} open={expanded.has(c.id)}
            onToggle={() => toggle(c.id)}
            busy={update.isPending}
            onUpdate={(patch) => update.mutate({ id: c.id, ...patch })}
            onReopen={async () => { if (await confirmDialog({ title: "Reopen this save case?", body: "This undoes the outcome and puts the case back on the board.", confirmLabel: "Reopen" })) reopen.mutate({ id: c.id }); }}
...
function MetricTile({ k, v, n, accent }: { k: string; v: React.ReactNode; n?: string; accent?: string }) {
  return (
    <div className="eh-card" style={{ padding: "1rem 1.1rem", borderLeft: accent ? `3px solid ${accent}` : undefined }}>
      ...
      <div className="eh-num" style={{ fontSize: "2rem", fontWeight: 800, lineHeight: 1, color: accent ?? "var(--eh-ink)" }}>{v}</div>
...
function StepDots({ done, total }: { done: number; total: number }) {
  return (
    <span className="eh-row" style={{ gap: 4, alignItems: "center" }} aria-label={`${done} of ${total} steps done`}>
      {Array.from({ length: total }).map((_, i) => (
        <i key={i} style={{
          width: 8, height: 8, borderRadius: "50%", display: "block",
          background: i < done ? "var(--eh-good, #2e7d5b)" : "var(--eh-line, #d8d2c4)",
        }} />
      ))}
    </span>
  );
...
function CaseCard({ c, me, open, onToggle, busy, onUpdate, onReopen, onOutcome }: {
  c: SaveCase; me: number | null; open: boolean; onToggle: () => void; busy: boolean;
  onUpdate: (patch: { ownerUserId?: number | null; stepsMask?: number; notes?: string }) => void;
  onReopen: () => void; onOutcome: (outcome: "saved" | "lost") => void;
}) {
  ...
  return (
    <div className="eh-card" style={{ padding: 0, overflow: "hidden", borderLeft: `3px solid ${accent}` }}>
      {/* header row — click to expand */}
      <button onClick={onToggle} aria-expanded={open}
        style={{ display: "flex", alignItems: "center", gap: ".85rem", width: "100%", textAlign: "left", flexWrap: "wrap",
          background: "none", border: 0, cursor: "pointer", padding: "1rem 1.2rem" }}>
        <span style={{ flex: "none", width: 40, height: 40, borderRadius: "50%", display: "grid", placeItems: "center",
          background: "var(--eh-ink-2, #16264c)", color: "#F3F1EA", fontWeight: 700, fontSize: ".85rem" }}>
          {initials(c.memberName)}
        </span>
        <span style={{ flex: "1 1 11rem", minWidth: 0 }}>
          <span className="eh-row" style={{ gap: ".5rem", alignItems: "center", flexWrap: "wrap" }}>
            <b style={{ fontSize: "1rem" }}>{c.memberName ?? `Member #${c.memberId}`}</b>
            {c.tier && <TierPill tier={c.tier} />}
            <Pill color={STATUS_COLOR[c.status]}>{STATUS_LABEL[c.status]}</Pill>
          </span>
          <span className="eh-muted eh-sm" style={{ display: "block", marginTop: ".2rem", whiteSpace: "nowrap",
            overflow: "hidden", textOverflow: "ellipsis" }}>
            {c.chapterName ? `${c.chapterName} · ` : ""}{c.reason}
          </span>
        </span>
        <span className="eh-row" style={{ flex: "none", gap: ".9rem", alignItems: "center", marginLeft: "auto" }}>
          {isOpen && (
            <span className="eh-sm" style={{ color: u.color, fontWeight: 600, whiteSpace: "nowrap" }}
              title={`Opened ${fmtDate(c.openedAt)}`}>{u.label}</span>
          )}
          <span className="eh-row" style={{ gap: ".45rem", alignItems: "center" }}>
            <StepDots done={c.stepsDone} total={c.stepsTotal} />
            <span className="eh-muted eh-sm eh-num" style={{ whiteSpace: "nowrap" }}>{c.stepsDone}/{c.stepsTotal}</span>
          </span>
          <span aria-hidden style={{ color: "var(--eh-muted, #5f5c53)", transition: "transform .2s",
            transform: open ? "rotate(90deg)" : "none", fontSize: ".9rem" }}>▸</span>
        </span>
      </button>

      {open && (
        <div style={{ padding: "0 1.2rem 1.2rem", borderTop: "1px solid var(--eh-line, #d8d2c4)" }}>
          <div className="eh-between" style={{ margin: ".9rem 0 .2rem", flexWrap: "wrap", gap: ".5rem" }}>
            <span className="eh-sm eh-muted">
              Opened {fmtDate(c.openedAt)}{c.closedAt ? ` · closed ${fmtDate(c.closedAt)}` : ""} · Owner: {c.ownerName ?? "—"}
            </span>
            {isOpen && (
              mine
                ? <button className="eh-btn ghost sm" disabled={busy} onClick={() => onUpdate({ ownerUserId: null })}>Unassign</button>
                : <button className="eh-btn ghost sm" disabled={busy} onClick={() => onUpdate({ ownerUserId: me })}>Assign to me</button>
            )}
          </div>
          ...
          <div className="eh-list" style={{ marginTop: ".4rem" }}>
            {SAVE_PLAYBOOK_STEPS.map((step, i) => {
              ...
              return (
                <button key={step.key} disabled={!isOpen || busy}
                  onClick={() => onUpdate({ stepsMask: c.stepsMask ^ (1 << i) })}
                  style={{
                  ...
                  <span>
                    <b style={{ fontSize: ".95rem", textDecoration: done ? "line-through" : "none", opacity: done ? .7 : 1 }}>{step.label}</b>
                    <span className="eh-sm eh-muted" style={{ display: "block" }}>{step.hint}</span>
                  </span>
                </button>
              );
            })}
          ...
          <NotesBox key={`n${c.id}`} initial={c.notes ?? ""} disabled={!isOpen || busy}
          ...
          {isOpen ? (
            <div className="eh-row" style={{ gap: ".5rem", marginTop: ".9rem" }}>
              <button className="eh-btn green sm" onClick={() => onOutcome("saved")}>Mark saved</button>
              <button className="eh-btn ghost sm" onClick={() => onOutcome("lost")}>Mark lost</button>
            </div>
          ) : (
            <div className="eh-row" style={{ gap: ".5rem", marginTop: ".9rem" }}>
              <button className="eh-btn ghost sm" onClick={onReopen}>Reopen</button>
            </div>
          )}
        </div>
      )}
