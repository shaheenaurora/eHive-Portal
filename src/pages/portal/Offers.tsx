...
export default function Offers() {
  ...
  return (
    <EhShell groups={MEMBER_NAV} brandSub="Member Portal">
      <PageHead eyebrow="Member offers" title="Your tier, working for you"
      ...
      {[["Business Setup", setup], ["Consulting", consulting]].map(([label, list]) => (
        (list as typeof setup).length > 0 && (
          <div key={label as string} className="eh-mb">
            <div className="eh-eyebrow" style={{ marginBottom: ".7rem" }}>{label as string}</div>
            <div className="eh-grid g3">
              {(list as typeof setup).map((o) => (
                <div className="eh-card" key={o.id} style={{ display: "flex", flexDirection: "column" }}>
                  <div className="eh-between">
                    <Pill color={o.vertical === "setup" ? "blue" : "purple"}>{o.vertical === "setup" ? "Setup" : "Consulting"}</Pill>
                    <TierPill tier={o.tierGate} />
                  </div>
                  <h3 className="eh-mt">{o.title}</h3>
                  <p className="eh-sm eh-muted" style={{ flex: 1 }}>{o.description}</p>
                  {o.ctaUrl && (
                    <a className="eh-btn gold sm eh-mt" href={o.ctaUrl} target="_blank" rel="noreferrer">Claim this offer →</a>
                  )}
                </div>
              ))}
            </div>
