  ...
  return (
    <EhShell groups={MEMBER_NAV} brandSub="Member Portal">
      <PageHead
        ...
        title="Member-built, member-only"
      ...
      {q.isLoading && <Spinner />}
      {q.isError && (
        <LoadError what="the library" onRetry={() => q.refetch()} />
      )}
      {q.data && list.length === 0 && (
        <div className="eh-card">
          <Empty big="Nothing matches." p="Try a different filter or search." />
        </div>
      )}
            ...
            <div className="eh-mt">
              {i.locked ? (
                <div className="eh-locked">
                  <Pill>{i.tierGate}+</Pill>
                  <span className="eh-sm">Unlocks at {i.tierGate} tier.</span>
                </div>
              ) : i.url ? (
                <a
