...
export default function Chapter() {
  ...
  const myTransfer = trpc.circle.myChapterTransfer.useQuery(undefined, {
    retry: false,
  });
  ...
  const chapterReqs = trpc.officer.chapterChangeRequests.useQuery(undefined, {
    retry: false,
    enabled: isOfficer,
  });
  const [signupSearch, setSignupSearch] = useState("");
  const candidates = trpc.officer.signupCandidates.useQuery(
    { q: signupSearch || undefined },
    { retry: false, enabled: isOfficer }
  );
  ...
  const decideChapterChange =
    trpc.officer.decideChapterMemberChange.useMutation({
      onSuccess: () => {
  ...
  const signup = trpc.officer.signupMember.useMutation({
    onSuccess: () => {
      toast("Member added to your chapter.");
      refresh();
    },
    onError: e => toast(e.message),
  });
  ...
  const postLearning = trpc.officer.postLearning.useMutation({
    onSuccess: () => {
      toast("Learning posted to your chapter.");
      setLearnTitle("");
      setLearnBody("");
      setLearnUrl("");
      refresh();
  ...
  const vote = trpc.engage.castVote.useMutation({
    onSuccess: () => {
      toast("Ballot cast — it's secret and counted.");
      refresh();
    },
    onError: e => toast(e.message),
  });
  const voteMotion = trpc.engage.voteMotion.useMutation({
    onSuccess: () => {
      toast("Vote recorded — one member, one vote.");
      refresh();
    },
    onError: e => toast(e.message),
  });
  ...
  const chapterName = (id: number | null | undefined) =>
    chapters.find(c => c.id === id)?.name ?? "another chapter";
  const pending = myTransfer.data ?? null;
  const options = chapters.filter(
    c => c.id !== (ch?.id ?? dir.data?.homeChapterId)
  );
    ...
    <EhShell groups={MEMBER_NAV} brandSub="Member Portal" notif>
      ...
      {pending && (
        <div className="eh-banner eh-mb">
          <span className="eh-sm">
            <b>Transfer to {chapterName(pending.toChapterId)}</b> is awaiting
            management approval.
          </span>
        </div>
      )}
      ...
      {ch && (
        <>
          <div className="eh-grid g3 eh-mb">
            ...
            <div className="eh-card eh-stat">
              ...
              <div className="v" style={{ fontSize: "1.1rem" }}>
                {[ch.zone, ch.city, ch.state, ch.country]
                  .filter(Boolean)
                  .join(", ") || "—"}
              </div>
            </div>
          ...
          {!pending && options.length > 0 && (
            <div className="eh-mb" style={{ marginTop: "-.25rem" }}>
              <button
                ...
                onClick={() => setTransferOpen(true)}
              >
                Request transfer to another chapter →
              </button>
            </div>
          )}
          ...
          {(q.data!.board ?? []).length > 0 && (
            <>
              <h2 className="eh-h2" style={{ margin: "1.5rem 0 .75rem" }}>
                Chapter leadership
              </h2>
              <div className="eh-card">
                  ...
                  {q.data!.board!.map(b => {
                    ...
                    return (
                      ...
                      >
                        <div style={{ flex: 1 }}>
                          <div className="t">
                            {chapterRoleTitle(b.role, b.title)} — {b.memberName}{" "}
                            {mine && <Pill color="green">you</Pill>}
                          </div>
                          <div className="d">
                            {b.responsibilities ||
                              CHAPTER_ROLE_RESP[b.role] ||
                              ""}
                          </div>
          ...
          {isOfficer && overview.data && (
            <>
              ...
              <p className="eh-sm eh-muted" style={{ marginTop: "-.4rem" }}>
                You lead this chapter. Sign up members, assign mentors, onboard
                newcomers and share learnings — scoped to {ch.name}.
              </p>
              ...
              {(overview.data.myRoles ?? []).map(r => {
                ...
                const done = ROLE_ONBOARDING_STEPS.filter(
                  (_, i) => (r.onboardingMask & (1 << i)) !== 0
                ).length;
                ...
                const roleName =
                  r.role === "other"
                    ? r.title || "Officer"
                    : CHAPTER_ROLE_LABEL[r.role] || r.role;
                return (
                    ...
                    >
                      <div>
                        ...
                        <div className="eh-muted eh-sm">
                          Your onboarding playbook — {done}/
                          {ROLE_ONBOARDING_STEPS.length} done.
                        </div>
                      </div>
                      ...
                      {ROLE_ONBOARDING_STEPS.map((s, i) => {
                        ...
                        return (
                          <button
                            key={s.key}
                            className="row"
                            disabled={updateRoleOnboarding.isPending}
                            onClick={() =>
                              updateRoleOnboarding.mutate({
                                roleId: r.id,
                                mask: r.onboardingMask ^ (1 << i),
                              })
                            }
                            style={{
                              display: "flex",
              ...
              {(chapterReqs.data ?? []).filter(r => r.status === "pending")
                .length > 0 && (
                <div
                ...
                >
                  ...
                  >
                    <div>
                      <h3 style={{ margin: 0 }}>Member change requests</h3>
                      <div className="eh-muted eh-sm">
                        Proposed changes for your chapter members — approve or
                        decline.
                      </div>
                    </div>
                    <Pill color="gold">
                      {
                        (chapterReqs.data ?? []).filter(
                          r => r.status === "pending"
                        ).length
                      }
                    </Pill>
                        ...
                        >
                          ...
                          <span className="eh-row" style={{ gap: ".3rem" }}>
                            <button
                              ...
                              disabled={decideChapterChange.isPending}
                              onClick={async () => {
                                if (
                                  await confirmDialog({
                ...
                <div className="eh-card eh-mb">
                  ...
                  >
                    <div>
                      ...
                      <div className="eh-muted eh-sm">
                        The number your chapter is measured on.
                      </div>
                    </div>
                    <div style={{ textAlign: "right" }}>
                      ...
                      >
                        {overview.data.health.total}
                      </div>
                      <Pill
                        color={
                          HEALTH_BAND_COLOR[
                            healthBand(overview.data.health.total)
                          ]
                        }
                      >
                        {
                  ...
                  >
                    {HEALTH_COMPONENTS.map(c => {
                      const v = (
                        overview.data!.health.components as Record<
                          string,
                          number
                        >
                      )[c.key];
                      return (
                        <div
                          ...
                          style={{
                            display: "grid",
                            gridTemplateColumns: "9.5rem 1fr 2.4rem",
                            alignItems: "center",
                            gap: ".5rem",
                          }}
                        >
              ...
              <div className="eh-card eh-mb">
                <div className="eh-between">
                  ...
                  {overview.data.cadence &&
                    overview.data.cadence.cadences.length > 0 && (
                      <Pill
                        color={
                          overview.data.cadence.adherence >= 80
                            ? "green"
                            : overview.data.cadence.adherence >= 60
                              ? "gold"
                              : "red"
                        }
                      >
                        {overview.data.cadence.adherence}% kept
                      </Pill>
                    )}
                </div>
                {!overview.data.cadence ||
                overview.data.cadence.cadences.length === 0 ? (
                  <div style={{ textAlign: "center", padding: ".4rem 0" }}>
                    <p className="eh-sm eh-muted">
                      Set your chapter's recurring rhythm — meetings, huddle,
                      board, financial close.
                    </p>
                    <button
                      className="eh-btn gold"
                      disabled={setupCadences.isPending}
                      onClick={() => setupCadences.mutate()}
                    >
                      ...
                      >
                        ...
                        <div className="eh-row" style={{ gap: ".4rem" }}>
                            ...
                            <>
                              <button
                                ...
                                onClick={async () => {
                                  if (
                                    await confirmDialog({
                                      title: `Mark this ${periodLabel(c.frequency as Frequency)} cadence as kept?`,
                                      body: "This records it as done for this period and rolls up to your Zone. You can undo it afterwards.",
                                      confirmLabel: "Yes, mark kept",
                                    })
                                  )
                                    markCadence.mutate({
                                      cadenceId: c.id,
                              ...
                              <button
                                ...
                                onClick={async () => {
                                  if (
                                    await confirmDialog({
                                      title:
                                        "Mark this cadence as rescheduled?",
                                      confirmLabel: "Yes, rescheduled",
                                    })
                                  )
                                    markCadence.mutate({
                                      cadenceId: c.id,
                          ...
                          {c.currentStatus !== "open" && (
                            <button
                              className="eh-btn sm ghost"
                              disabled={reopenCadence.isPending}
                              onClick={async () => {
                                if (
                                  await confirmDialog({
                                    title: `Undo "${c.currentStatus}" for this period?`,
                                    body: "This reopens the cadence so it can be marked again.",
                                    confirmLabel: "Undo",
                                    danger: true,
                                  })
                                )
                                  reopenCadence.mutate({ cadenceId: c.id });
                              }}
                            >
                              Undo
                            </button>
              ...
              {(overview.data.onboarding ?? []).length > 0 && (
                <div className="eh-card eh-mb">
                  ...
                  <p className="eh-sm eh-muted" style={{ marginTop: 0 }}>
                    New members and how far through onboarding they are. POD
                    placement is due by day 60.
                  </p>
                  <div className="eh-list">
                    {overview.data.onboarding.map(o => (
                      <div className="row" key={o.id}>
                        <div style={{ flex: 1 }}>
                          ...
                          <div className="d">
                            Day {o.dayCount} · {o.doneCount}/{o.total}{" "}
                            milestones
                            {o.dayCount > 60 && o.stage < 3 ? " · behind" : ""}
                          </div>
                        </div>
                        ...
                        <span
                          className="eh-num eh-sm"
                          style={{ width: "2.4rem", textAlign: "right" }}
                        >
                          {o.percent}%
                        </span>
                      </div>
              ...
              <div className="eh-grid g2" style={{ alignItems: "start" }}>
                ...
                <div className="eh-card">
                  ...
                  <p className="eh-sm eh-muted" style={{ marginTop: 0 }}>
                    Add an unassigned member to your chapter.
                  </p>
                  <Field label="Search">
                    <input
                      className="eh-input"
                    ...
                    {(candidates.data ?? []).map(c => (
                      <div className="row" key={c.id}>
                        ...
                        <button
                          ...
                          onClick={() => signup.mutate({ memberId: c.id })}
                        >
                          Add →
                        </button>
                      </div>
                ...
                <div className="eh-card">
                  ...
                  <p className="eh-sm eh-muted" style={{ marginTop: 0 }}>
                    Pair a member with a mentor from your chapter.
                  </p>
                  <Field label="Member (mentee)">
                    ...
                    >
                      ...
                      {overview.data.roster.map(m => (
                        <option key={m.id} value={m.id}>
                          {m.name}
                          {m.hasMentor ? " (has mentor)" : ""}
                        </option>
                      ))}
                    </select>
                  </Field>
                  <Field label="Mentor">
                    ...
                    >
                      <option value="">Select…</option>
                      {overview.data.roster
                        .filter(m => String(m.id) !== mentee)
                        .map(m => (
                          <option key={m.id} value={m.id}>
                            {m.name}
                          </option>
                  ...
                  <button
                    className="eh-btn gold"
                    disabled={assignMentor.isPending || !mentee || !mentor}
                    onClick={() =>
                      assignMentor.mutate({
              ...
              <div className="eh-card eh-mt">
                ...
                <div className="eh-list">
                  {overview.data.roster.filter(m => !m.hasMentor).length ===
                    0 && (
                    <div className="eh-sm eh-muted">
                      Everyone in your chapter has a mentor. 🎉
                    </div>
                  )}
                  {overview.data.roster
                    .filter(m => !m.hasMentor)
                    .map(m => (
                      <div className="row" key={m.id}>
                        <div style={{ flex: 1 }}>
                          <div className="t">{m.name}</div>
                          <div className="d">{m.company ?? ""}</div>
                        </div>
                        <button
                          className="eh-btn ghost sm"
                          onClick={() => {
                            setMentee(String(m.id));
                            setMentor("");
                          }}
                        >
                          Assign mentor →
              ...
              <div className="eh-card eh-mt">
                ...
                <Field label="Title">
                  <input
                    ...
                    placeholder="e.g. How we filled 3 seats last month"
                  />
                </Field>
                ...
                <button
                  className="eh-btn gold"
                  disabled={
                    postLearning.isPending || learnTitle.trim().length < 3
                  }
                  onClick={() =>
                    postLearning.mutate({
                      title: learnTitle.trim(),
                      body: learnBody || undefined,
                      url: learnUrl || undefined,
                    })
                  }
                >
                  Post to chapter
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        {l.url && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            style={{ color: "var(--eh-gold)" }}
                          >
                            Open link →
                          </a>
                        )}
                        <div className="d eh-muted">
                          {l.authorName} · {fmtDate(l.createdAt)}
                        </div>
                      </div>
                      {isOfficer && (
                        <button
                          ...
                          onClick={async () => {
                            if (
                              await confirmDialog({
                                ...
                                body: `"${l.title}" will be removed from the chapter learnings.`,
                                danger: true,
                                confirmLabel: "Delete",
                              })
                            ) {
                              deleteLearning.mutate({ id: l.id });
                            }
                          }}
                        >
                          Remove
          ...
          {(q.data!.elections ?? []).length === 0 && (
            <div className="eh-card">
              <Empty
                big="No elections right now."
                p="Chapter seats open every term — nominations are announced here."
              />
            </div>
          )}
            ...
            <div className="eh-card eh-mb" key={e.id}>
              <div className="eh-between">
                <div>
                  ...
                  <div className="eh-muted eh-sm">
                    Seat: {e.seat} · Quorum {e.quorumPct}% · Turnout {e.turnout}
                    /{q.data!.memberCount}
                  </div>
                </div>
                {e.status === "open" && (
                  <Pill color="blue">nominations open</Pill>
                )}
              ...
              <div className="eh-list eh-mt">
                    ...
                    >
                      ...
                      {e.status === "voting" && !e.voted && (
                        <button
                          ...
                          onClick={() =>
                            vote.mutate({ electionId: e.id, candidateId: c.id })
                          }
                        >
                          Vote
                        </button>
                ...
                {e.candidates.length === 0 && (
                  <p className="eh-muted eh-sm">No candidates yet.</p>
                )}
              </div>
              ...
              >
                ...
                {e.status === "voting" && e.voted && (
                  <Pill color="green">ballot cast ✓</Pill>
                )}
          ...
          {(q.data!.motions ?? []).length === 0 && (
            <div className="eh-card">
              <Empty
                big="No open motions."
                p="Chapter motions are tabled by the chapter board."
              />
            </div>
          )}
                ...
                >
                  {m.status === "open" && !m.myChoice && (
                    <>
                      <button
                        className="eh-btn sm gold"
                        disabled={voteMotion.isPending}
                        onClick={() =>
                          voteMotion.mutate({ motionId: m.id, choice: "yes" })
                        }
                      >
                        Yes
                      </button>
                      <button
                        ...
                        disabled={voteMotion.isPending}
                        onClick={() =>
                          voteMotion.mutate({ motionId: m.id, choice: "no" })
                        }
                      >
                        No
                      </button>
                  ...
                  {m.myChoice && (
                    <Pill color="purple">you voted {m.myChoice}</Pill>
                  )}
                  <span className="eh-muted eh-sm">
                ...
                <div className="eh-list">
                  {q.data!.budgets!.map(b => (
                    <div className="row" key={b.id}>
                      ...
                      {b.status === "spent" && (
                        <Pill color="purple">spent</Pill>
                      )}
                      {b.status === "proposed" && (
                        <Pill color="blue">proposed</Pill>
                      )}
                    </div>
                  ))}
                </div>
      ...
      {standFor !== null && (
        <Modal title="Stand for election" onClose={() => setStandFor(null)}>
          <p className="eh-sm eh-muted">
            Your name goes on the ballot for this seat. Voting is by secret
            ballot with a quorum requirement.
          </p>
          <Field label="Your statement (why you?)">
            <textarea
              ...
              maxLength={1000}
              placeholder="What you'd do for the chapter."
            />
          </Field>
          <button
            className="eh-btn gold"
      ...
      {transferOpen && (
        <Modal
          title={
            ch ? "Request a chapter transfer" : "Request to join a chapter"
          }
          onClose={() => setTransferOpen(false)}
        >
          <p className="eh-sm eh-muted" style={{ marginTop: 0 }}>
            {ch
              ? "Choose the chapter you'd like to move to. "
              : "Choose the chapter you'd like to join. "}
            The Circle team reviews and confirms every placement — your home
            chapter changes once it's approved.
          </p>
          <Field label="Chapter">
            <select
              className="eh-select"
            ...
            >
              ...
              {options.map(c => (
                <option key={c.id} value={c.id}>
                  {c.name}
                  {[c.zone, c.city, c.country].filter(Boolean).length
                    ? ` — ${[c.zone, c.city, c.country].filter(Boolean).join(", ")}`
                    : ""}
                </option>
              ))}
            </select>
          ...
          <Field label="Reason (optional)">
            <textarea
              ...
              maxLength={500}
              placeholder="e.g. I've relocated to Abu Dhabi."
            />
          </Field>
          <button
            className="eh-btn gold"
            style={{ width: "100%" }}
            disabled={requestTransfer.isPending || !toChapter}
            onClick={() =>
              requestTransfer.mutate({
                toChapterId: Number(toChapter),
