    ...
    <EhShell groups={MEMBER_NAV} brandSub="Member Portal">
      ...
      <div className="eh-grid g3">
        {q.data?.map(({ pod, role, memberCount, nextSession }) => (
          <Link
            key={pod.id}
            to={`/portal/pods/${pod.id}`}
            style={{ textDecoration: "none", color: "inherit" }}
          >
            <div
              className="eh-card"
              style={{ height: "100%", transition: "border-color .15s" }}
              onMouseEnter={e =>
                (e.currentTarget.style.borderColor = "var(--eh-gold)")
              }
            ...
            >
              ...
              <p className="eh-sm eh-muted" style={{ minHeight: "2.6em" }}>
                {pod.description}
              </p>
