...
export default function PodDetail() {
  ...
  const accept = trpc.circle.acceptPodConfidentiality.useMutation({
    onSuccess: () => {
      toast("Thank you — welcome to the pod.");
      utils.circle.podDetail.invalidate({ id: podId });
    },
    onError: e => toast(e.message),
  });
  ...
  if (q.error || !q.data)
    return (
      <EhShell groups={MEMBER_NAV} brandSub="Member Portal">
        <Empty
          big="This pod isn't open to you."
          p="If you think that's a mistake, message your facilitator."
        >
          <Link className="eh-btn ghost sm" to="/portal/pods">
            ← Back to my pods
          </Link>
        </Empty>
      </EhShell>
  ...
  const upcoming = sessions.filter(
    s => new Date(s.startsAt).getTime() >= now && s.status === "scheduled"
  );
  const past = sessions.filter(s => !upcoming.includes(s));
  const myOpen = actionItems.filter(
    a => a.item.memberId === me.id && a.item.status === "open"
  );
      ...
      <div className="eh-page-head">
        <div>
          <div className="eh-eyebrow">
            <Link to="/portal/pods" style={{ color: "inherit" }}>
              ← Pods
            </Link>
          </div>
      ...
      {!confidentialityAccepted ? (
        <div className="eh-card" style={{ maxWidth: 640 }}>
          ...
          <p className="eh-sm eh-muted">
            What's said in a POD stays in the POD. It's the trust the whole
            thing runs on. Before you can see this pod's sessions, commitments
            and notes, please confirm you'll keep everything shared here
            strictly confidential — a breach is a conduct matter.
          </p>
          <button
            className="eh-btn gold"
            disabled={accept.isPending}
            onClick={() => accept.mutate({ podId })}
          >
        ...
        <>
          ...
          >
            ...
            <span>
              <b>Chatham House Rule.</b> Everything shared in this room —
              numbers, decisions, who said what — stays in it.
            </span>
          </div>
            ...
            >
              ...
              <div className="eh-card">
                ...
                {sessions.length === 0 && (
                  <Empty
                    big="No sessions yet."
                    p="Your facilitator will schedule the first one."
                  />
                )}
                <div className="eh-list">
                  {upcoming.map(s => (
              ...
              <div className="eh-card">
                ...
                {actionItems.length === 0 && (
                  <Empty big="Nothing on the board." />
                )}
                <div className="eh-list">
                  {actionItems.map(({ item, user }) => (
                    <div className="row" key={item.id}>
                      <div>
                        ...
                        <div className="d">
                          {user.name ?? "Member"} · due {relDay(item.dueAt)}
                        </div>
                      </div>
            ...
            <div className="eh-card">
              ...
              <div className="eh-list">
                <div className="row">
                  ...
                  <span className="t eh-sm">{pod.facilitator ?? "—"}</span>
                </div>
                <div className="row">
                  <span className="d">Cadence</span>
                  <span className="t eh-sm">{pod.cadence ?? "—"}</span>
                </div>
              </div>
            </div>
          </div>
