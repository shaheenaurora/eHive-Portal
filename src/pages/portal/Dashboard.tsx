...
export default function Dashboard() {
  ...
  const completeStep = trpc.circle.completeOnboardingStep.useMutation({
    onSuccess: r => {
      if (r.already) {
        toast("That step is already marked done.");
      } else if (r.complete) {
        toast("Onboarding complete — welcome to full membership!");
      } else {
        toast("Nice — step done.");
      }
  ...
  if (!d.data)
    return (
      <EhShell groups={MEMBER_NAV} brandSub="Member Portal" notif>
        <LoadError what="your dashboard" onRetry={() => d.refetch()} />
      </EhShell>
    );

  const { member, nextSession, openActionItems, upcomingEvents, podCount } =
    d.data;
  const first = (user?.name ?? "there").split(" ")[0];
  const showOnboarding =
    ob.data && !ob.data.complete && ob.data.lifecycleState === "onboarding";
    ...
    <EhShell groups={MEMBER_NAV} brandSub="Member Portal" notif>
        ...
        <div className="eh-banner eh-mb">
          <div className="eh-between" style={{ alignItems: "flex-start" }}>
            <div>
              <div className="eh-eyebrow" style={{ color: "var(--eh-gold-2)" }}>
                Your first 90 days · Day {ob.data!.dayCount}
              </div>
              <h2 style={{ margin: ".2rem 0 .1rem" }}>
                Onboarding · {ob.data!.doneCount} of {ob.data!.total}
              </h2>
              <p className="eh-sm" style={{ marginBottom: 0 }}>
                Members who complete this are far more likely to thrive. POD
          ...
          <div style={{ marginTop: ".7rem", display: "grid", gap: ".9rem" }}>
            {ONBOARDING_STAGES.map(st => {
              const items = ob.data!.milestones.filter(
                m => m.stage === st.stage
              );
              return (
                <div key={st.stage}>
                  ...
                  <div className="eh-list">
                    {items.map(m => (
                      <div className="row" key={m.key}>
                        <span style={{ fontSize: "1.05rem", width: "1.5rem" }}>
                          {m.done ? "✅" : "⬜"}
                        </span>
                        <span
                          className="t"
                        ...
                        {!m.done && !m.auto && (
                          <button
                            ...
                            onClick={() =>
                              completeStep.mutate({ milestone: m.key })
                            }
                          >
                            Mark done
                          </button>
      ...
      <div className="eh-grid g4">
        ...
        <Stat
          k="Open commitments"
          v={openActionItems}
          n="Action items owed to your pods"
        />
        <Stat
          k="Events booked"
          v={upcomingEvents.length}
          n="Your upcoming registrations"
        />
      </div>
      ...
      <div className="eh-grid g3 eh-mt" style={{ alignItems: "start" }}>
        ...
        >
          {nextSession ? (
            <div className="eh-banner">
              ...
              <p>
                {nextSession.pod.name} · {fmtDay(nextSession.session.startsAt)}{" "}
                · {fmtDateTime(nextSession.session.startsAt).split("·")[1]}
              </p>
              <div className="eh-row eh-mt">
                {nextSession.session.videoLink && (
                  <a
                    className="eh-btn gold sm"
                    href={nextSession.session.videoLink}
                    target="_blank"
                    rel="noreferrer"
                  >
                    Join video room →
            ...
            <div className="eh-banner">
              ...
              <p>
                Your facilitator schedules pod sessions — check back soon, or
                browse the events calendar meanwhile.
              </p>
            </div>
          )}
          ...
          <div className="eh-card">
            <div className="eh-between">
              ...
              <Link className="eh-btn ghost sm" to="/portal/events">
                All events →
              </Link>
            </div>
            {upcomingEvents.length === 0 ? (
              <Empty
                big="Nothing booked yet."
                p="Spark Evenings, Circle Dinners and retreats — the calendar fills fast."
              >
                <Link className="eh-btn gold sm" to="/portal/events">
                  Browse events
                </Link>
              </Empty>
            ) : (
                ...
                {upcomingEvents.map(({ event, reg }) => (
                  <div className="row" key={reg.id}>
                    <div>
                      ...
                      <div className="d">
                        {fmtDay(event.startsAt)} · {event.location ?? "TBA"}
                      </div>
                    </div>
                    <Pill color={reg.status === "attended" ? "green" : "blue"}>
                      {reg.status}
                    </Pill>
                  </div>
                ))}
        ...
        <div className="eh-card" style={{ textAlign: "center" }}>
          ...
          <div
            style={{ display: "grid", placeItems: "center", margin: ".6rem 0" }}
          >
            <Ring value={member.hiveScore} />
          </div>
          <p className="eh-sm eh-muted" style={{ margin: "0 0 .8rem" }}>
            Attendance, commitments, events, contribution, FRP progress and
            tenure — the six factors that keep the circle strong.
          </p>
          <Link className="eh-btn ghost sm" to="/portal/score">
            Score breakdown →
          </Link>
        </div>
      </div>
