...
function VerifyBanner() {
  ...
  const cfg = trpc.auth.config.useQuery(undefined, {
    retry: false,
    staleTime: 5 * 60 * 1000,
  });
  ...
  return (
    <div className="eh-verify" role="status">
      ...
      <span style={{ flex: 1 }}>
        <b>Confirm your email</b> to secure your account. We sent you a link —
        didn't get it?{" "}
        <button
...
let openConfirmFn:
  ((opts: ConfirmOpts, resolve: (v: boolean) => void) => void) | null = null;
...
function ConfirmHost() {
  ...
  return (
    <Modal title={opts.title} onClose={() => done(false)}>
      {opts.body && (
        <p className="eh-sm eh-muted" style={{ marginBottom: "1.1rem" }}>
          {opts.body}
        </p>
      )}
      <div className="eh-row" style={{ gap: ".5rem" }}>
        <button
          className={"eh-btn " + (opts.danger ? "" : "gold")}
          style={
            opts.danger
              ? {
                  background: "var(--eh-red)",
                  color: "#fff",
                  borderColor: "var(--eh-red)",
                }
              : undefined
          }
          onClick={() => done(true)}
        ...
        <button className="eh-btn ghost" onClick={() => done(false)}>
...
type PillColor =
  "gold" | "green" | "red" | "amber" | "blue" | "purple" | "grey";
export function Pill(props: { color?: PillColor; children: ReactNode }) {
  return (
    <span
      className={
        "eh-pill" +
        (props.color && props.color !== "gold" ? " " + props.color : "")
      }
    >
      {props.children}
    </span>
...
export function TierPill(props: { tier: string }) {
  ...
  return (
    <Pill color={map[props.tier] ?? "grey"}>
      {TIER_LABEL[props.tier as keyof typeof TIER_LABEL] ?? props.tier}
    </Pill>
  );
}
...
export function StatusPill(props: { status: string }) {
  ...
  return (
    <Pill color={map[props.status] ?? "grey"}>
...
export function LoadError(props: { onRetry?: () => void; what?: string }) {
  return (
    <div className="eh-card">
      <Empty
        big={`Couldn't load ${props.what ?? "this"}.`}
        p="Something went wrong reaching the server. Check your connection and try again."
      >
        {props.onRetry && (
          <button className="eh-btn ghost" onClick={props.onRetry}>
            Try again
          </button>
        )}
      </Empty>
...
export function adminHasScope(
  adminScopes: string | null | undefined,
  scope: string
): boolean {
...
export function filterNavByScope(
  groups: NavGroup[],
  adminScopes: string | null | undefined
): NavGroup[] {
  return groups
    .map(g => ({
      ...g,
      items: g.items.filter(it => navItemVisible(it, adminScopes)),
    }))
...
export function EhShell(props: {
  ...
  return (
    <div className="eh-shell">
      ...
      {menuOpen && (
        <div className="eh-side-veil" onClick={() => setMenuOpen(false)} />
      )}
      {/* Any tap on a nav link or the foot links closes the mobile drawer. */}
      <aside
        id={sideId}
        className={"eh-side" + (menuOpen ? " open" : "")}
        onClick={e => {
      ...
      >
        <div className="eh-side-brand">
          <div className="eh-between" style={{ alignItems: "flex-start" }}>
            ...
            <button
              className="eh-btn ghost sm eh-side-close"
        ...
        <div className="eh-side-foot">
          ...
          <button className="eh-signout" onClick={() => logout()}>
            Sign out
          </button>
        </div>
      </aside>
...
export const ADMIN_NAV: NavGroup[] = [
  {
    items: [
      ...
      {
        to: "/admin/admissions",
        label: "Zenith & Investors",
        icon: "✧",
        scope: ["member_success", "partnerships"],
      },
      {
        to: "/admin/members",
        label: "Members",
  ...
  },
  {
    ...
    items: [
      { to: "/admin/ops", label: "Operations", icon: "⚙", scope: "full" },
      {
        ...
        label: "Reports & KPIs",
        icon: "▧",
        scope: ["full", "chapters", "membership", "member_success"],
      },
      {
        to: "/admin/conduct",
        label: "Conduct & Safeguarding",
        icon: "⚖",
        scope: "conduct",
      },
