...
export function KpiAlertsBanner() {
  ...
  const ack = trpc.admin.acknowledgeKpiAlert.useMutation({
    onSuccess: () => {
      toast("Acknowledged.");
      utils.admin.kpiAlerts.invalidate();
    },
    onError: e => toast(e.message),
  });
  const evalNow = trpc.admin.evaluateKpiAlerts.useMutation({
    onSuccess: r => {
      toast(`Checked — ${r.opened} new, ${r.resolved} cleared.`);
      utils.admin.kpiAlerts.invalidate();
    },
    onError: e => toast(e.message),
  });
  ...
  return (
    <div
      ...
      style={{
        borderColor: "#e6cdbf",
        background: "var(--eh-red-soft, #f1e6de)",
      }}
    >
      <div
        className="eh-between"
        style={{ marginBottom: ".5rem", flexWrap: "wrap", gap: ".5rem" }}
      >
        <div className="eh-eyebrow" style={{ color: "var(--eh-red, #B05C3E)" }}>
          KPI alerts · {rows.length} active
        </div>
        <button
        ...
        >
          {evalNow.isPending ? "Checking…" : "Re-check now"}
        </button>
      </div>
          ...
          <div className="row" key={a.id} style={{ alignItems: "flex-start" }}>
            <div style={{ flex: 1 }}>
              ...
              <div className="d eh-muted">
                Raised {fmtDate(a.createdAt)}
                {a.status === "acknowledged" && a.acknowledgedByEmail
