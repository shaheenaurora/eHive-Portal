...
export function PushSettings() {
  ...
  const keyQ = trpc.engage.pushKey.useQuery(undefined, { retry: false, enabled: supported });
  const catQ = trpc.engage.pushCategories.useQuery({ endpoint: endpoint ?? "" }, { enabled: !!endpoint, retry: false });
  ...
  useEffect(() => {
    if (!supported) return;
    getExistingSubscription().then((s) => setEndpoint(s?.endpoint ?? null)).catch(() => {});
  }, [supported]);
  ...
  if (!supported) {
    return (
      <div className="eh-card">
        <h3>Notifications</h3>
        <p className="eh-sm eh-muted">This device or browser doesn't support push notifications.</p>
      </div>
    );
  }

  const enabled = !!endpoint;
  const cats: string[] = catQ.data?.categories ?? PUSH_CATEGORIES.map((c) => c.key);
  ...
  async function enable() {
    ...
    try {
      ...
      if (!sub) { toast("Permission denied — allow notifications in your browser settings."); return; }
  ...
  async function disable() {
    ...
    } catch { toast("Could not turn off notifications."); }
    finally { setBusy(false); }
  }
  ...
  return (
    <div className="eh-card">
      ...
      {!enabled ? (
        <>
          <p className="eh-sm eh-muted">Session reminders, event nudges, waitlist promotions and renewal alerts — on this device.</p>
          <button className="eh-btn gold" disabled={busy || keyQ.isLoading} onClick={enable}>
            {busy ? "Enabling…" : "Enable notifications"}
          </button>
        </>
      ) : (
        <>
          <p className="eh-sm eh-muted">On for this device. Choose what you'd like to be notified about:</p>
          ...
          <button className="eh-btn ghost sm" disabled={busy} onClick={disable} style={{ marginTop: ".5rem" }}>
