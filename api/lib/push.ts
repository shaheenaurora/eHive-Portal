...
async function ensureVapid() {
  ...
  const db = getDb();
  const row = (await db.select().from(schema.appConfig).where(eq(schema.appConfig.key, "vapid")).limit(1)).at(0);
...
export async function pushToMember(memberId: number, payload: PushPayload, category?: string): Promise<void> {
  try {
    await ensureVapid();
    const db = getDb();
    ...
    await Promise.all(subs.map(async (s) => {
      if (category && s.categories) {
        ...
        } catch { /* malformed → treat as all-on */ }
      }
      try {
