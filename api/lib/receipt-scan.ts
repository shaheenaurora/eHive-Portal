...
const ALLOWED_MAGIC = [
  ...
  { mime: "image/webp", prefix: Buffer.from("RIFF"), offset: 8, extra: Buffer.from("WEBP") },
];
...
export async function scanReceipt(
  ...
  if (buffer.length > 6_000_000) return { ok: false, reason: "Receipt exceeds size limit." };

  if (!looksSafe(buffer)) {
    return { ok: false, reason: "Receipt file type does not match its contents." };
  }
    ...
    try {
      ...
      const res = await fetch(url, {
        ...
        headers: {
          ...
          ...(env.clamavToken ? { Authorization: `Bearer ${env.clamavToken}` } : {}),
        },
        body: buffer,
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = (await res.json()) as { success: boolean; infected?: boolean; result?: string };
      ...
      logger.error("[receipt-scan] ClamAV scan failed", { error: String(err), name });
      return { ok: false, reason: "Receipt could not be scanned. Try again or contact support." };
    }
  } else {
    logger.info("[receipt-scan] no AV configured; magic-byte check passed", { name });
  }
