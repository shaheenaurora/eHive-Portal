...
export type Actor = { id: number; email: string; role: string; adminScopes?: string | null };
export type ChangeCategory = "profile" | "tier" | "status" | "lifecycle" | "chapter";
...
export type Activity = { at: Date; kind: string; icon: string; title: string; detail?: string; actor?: string };

/** High-impact categories always route through approval and require a reason. */
export const HIGH_IMPACT: ReadonlySet<ChangeCategory> = new Set(["tier", "status", "lifecycle"]);

/** Editable member-profile fields → label + which table the column lives on. */
export const PROFILE_FIELDS: Record<string, { label: string; table: "users" | "members" }> = {
...
export function violatesFourEyes(actorUserId: number, requestedByUserId: number): boolean {
...
export function canApprove(actor: Actor, opts: { leadsMemberChapter: boolean }): boolean {
  if (actor.role === "admin") {
    const s = (actor.adminScopes ?? "").trim();
    if (s === "" || s === "*") return true;
    if (s.split(",").map((x) => x.trim()).includes("membership")) return true;
  }
