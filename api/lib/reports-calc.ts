...
export type Kpi = {
  ...
  target: string; status: Rag; family: KpiFamily;
};

/** RAG for a "higher is better" KPI (green at/over target, amber within band). */
export function ragAtLeast(value: number | null, target: number, amberBand = 0.9): Rag {
...
export function ragAtMost(value: number | null, target: number, amberBand = 1.1): Rag {
  if (value == null) return "none";
  if (value <= target) return "green";
  if (value <= target * amberBand) return "amber";
  return "red";
