...
export async function createAuthToken(userId: number, kind: "verify" | "reset", ttlMs: number): Promise<string> {
  const raw = randomBytes(32).toString("base64url");
  await getDb().insert(schema.authTokens).values({
    userId,
    kind,
...
export async function consumeAuthToken(raw: string, kind: "verify" | "reset"): Promise<number | null> {
  const db = getDb();
  const row = (
    await db
      .select()
