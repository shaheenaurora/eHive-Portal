...
export type CadenceView = {
  ...
  sop: string | null;
  currentKey: string;
  currentStatus: CadenceStatus | "open";
  kept: number;
  expected: number;
  missed: number;
  adherence: number; // over the last WINDOW periods
};

/** Set a chapter up to standard — seed the standard cadences it's missing. */
export async function ensureCadenceTemplates(
  chapterId: number
): Promise<number> {
  const db = getDb();
  ...
  if (toAdd.length) {
    await db.insert(schema.cadences).values(
      toAdd.map(t => ({
        chapterId,
        type: t.type,
        title: t.title,
...
export async function listCadences(
  chapterId: number,
  now = new Date()
): Promise<{ cadences: CadenceView[]; adherence: number }> {
  const db = getDb();
  const rows = await db
    .select()
    .from(schema.cadences)
  ...
  const logs = await db
    ...
    .where(inArray(schema.cadenceLog.cadenceId, ids));
  ...
  const out: CadenceView[] = rows.map(c => {
    ...
    for (const k of history) {
      const s = log.get(k);
      if (s === "kept" || s === "rescheduled") kept++;
    }
    const expected = history.length;
    const missed = expected - kept;
    const currentStatus = (log.get(current) ?? "open") as
      CadenceStatus | "open";
    totalKept += kept;
  ...
  return {
    ...
    adherence: totalExpected
      ? Math.round((totalKept / totalExpected) * 100)
      : 100,
  };
}

/** Record the current period of a cadence as kept / rescheduled / missed. */
export async function recordCadence(
  cadenceId: number,
  ...
  const cad = (
    await db
      .select()
      .from(schema.cadences)
      .where(eq(schema.cadences.id, cadenceId))
      .limit(1)
  ).at(0);
  ...
  await db
    ...
    .where(
      and(
        eq(schema.cadenceLog.cadenceId, cadenceId),
        eq(schema.cadenceLog.periodKey, current)
      )
    );
  await db
    .insert(schema.cadenceLog)
    .values({ cadenceId, periodKey: current, status, note, actorMemberId });
