  ...
  const rows = await getDb()
    ...
    .where(
      and(
        ...
        or(eq(schema.users.adminScopes, ""), eq(schema.users.adminScopes, "*"))
      )
    );
...
export async function evaluateKpiAlerts(): Promise<{
  opened: number;
  resolved: number;
}> {
  const db = getDb();
  ...
  const open = await db
    ...
    .where(
      and(
        eq(schema.kpiAlerts.scope, "network"),
    ...
    if (k.status === "red") {
      ...
      if (a) {
        await db
          ...
          .where(eq(schema.kpiAlerts.id, a.id));
        resolved++;
      }
    }
  }
    ...
    if (to.length) {
      ...
      await Promise.all(
        to.map(addr =>
          sendMail({
            to: addr,
            subject: `eHive KPI alert — ${newLines.length} metric${newLines.length === 1 ? "" : "s"} below bar`,
            html,
          }).catch(() => false)
        )
      );
...
export async function listKpiAlerts() {
  return getDb()
    ...
    .orderBy(desc(schema.kpiAlerts.createdAt))
    .limit(50);
}

/** Acknowledge an alert (it stays visible but marked seen until it resolves). */
export async function acknowledgeKpiAlert(
  actor: Actor,
  id: number
): Promise<{ ok: true }> {
  await getDb()
    ...
    .where(
      and(eq(schema.kpiAlerts.id, id), eq(schema.kpiAlerts.status, "open"))
    );
  return { ok: true };
}
