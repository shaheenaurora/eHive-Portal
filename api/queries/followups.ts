...
export async function createFollowUp(input: {
  ...
  const dueAt =
    input.dueInHours != null
      ? new Date(Date.now() + input.dueInHours * 3600_000)
      : null;
...
export async function openGuestFollowUp(
  prospectId: number,
  name: string,
  chapterId?: number | null,
  ownerUserId?: number | null
  ...
  return createFollowUp({
    ...
    chapterId,
    ownerUserId,
    dueInHours: GUEST_FOLLOWUP_HOURS,
  });
}
...
export async function listFollowUps(
  opts: { status?: "open" | "all"; chapterId?: number } = {}
): Promise<FollowUpRow[]> {
  const conds = [];
  if ((opts.status ?? "open") === "open")
    conds.push(eq(schema.followUps.status, "open"));
  if (opts.chapterId != null)
    conds.push(eq(schema.followUps.chapterId, opts.chapterId));
  const rows = await getDb()
    .select({
      id: schema.followUps.id,
      title: schema.followUps.title,
      chapterId: schema.followUps.chapterId,
      chapterName: schema.chapters.name,
      prospectId: schema.followUps.prospectId,
      ownerUserId: schema.followUps.ownerUserId,
    ...
    .leftJoin(
      schema.chapters,
      eq(schema.chapters.id, schema.followUps.chapterId)
    )
  ...
  return rows.map(r => ({
    ...r,
    overdue: r.dueAt ? new Date(r.dueAt).getTime() < now : false,
  }));
}
  ...
  await getDb()
    ...
    .where(
      and(
        ...
        inArray(schema.followUps.status, ["open"])
      )
    );
}
