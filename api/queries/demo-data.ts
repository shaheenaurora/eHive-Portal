...
const FIRST = ["Omar", "Sara", "Layla", "Yousef", "Amal", "Noor", "Rami", "Farah", "Hassan", "Zaid",
  "Lena", "Tariq", "Huda", "Faisal", "Grace", "Mira", "Karim", "Dana", "Sami", "Reem", "Bilal", "Aisha",
  "Khalid", "Maya", "Nadia", "Adam", "Salma", "Rashid", "Hind", "Jad", "Yara", "Tamer", "Lina", "Ziad",
  "Rana", "Marwan", "Dina", "Fadi", "Nour", "Kareem", "Sana", "Basel", "Rula", "Nabil", "Leen", "Hadi"];
const LAST = ["Haddad", "Iqbal", "Nasser", "Rahal", "Odeh", "Aziz", "Ali", "Khoury", "Saleh", "Mansour",
  "Barakat", "Fares", "Darwish", "Sabbagh", "Halabi", "Karam", "Sayegh", "Nabhan", "Ghanem", "Toma",
  "Shaheen", "Antar", "Zahra", "Murad", "Qasim", "Bishara", "Habib", "Salti", "Attar", "Kanaan"];
const COMPANIES = ["Northwind", "FinTech Labs", "Souk Retail", "Sabil", "Paloma", "BoltGrid", "Ledgerly",
  "DriftCo", "Verdeco", "NorthStar", "Tazej", "Lumen", "GreenField", "AtlasPay", "Kurent", "DatePalm",
  "Marid", "Anwar", "Falcon Logistics", "Cedar Labs", "Oasis Health", "Dunes Media", "Pearl Realty"];
const SECTORS = ["FinTech", "Logistics", "Retail", "HealthTech", "Hospitality", "Energy", "Media",
  "Real Estate", "Education", "F&B"];
const TITLES = ["Founder", "CEO", "Co-founder", "Managing Director", "COO", "Founder & CEO", "Partner"];
...
const HIERARCHY = {
  ...
  regions: [
    { name: "Dubai Region", code: "DEMO-R-DXB", zones: [
      { name: "DIFC Zone", code: "DEMO-Z-DIFC", chapters: ["Dubai · DIFC", "Dubai · Business Bay"] },
      { name: "Marina Zone", code: "DEMO-Z-MAR", chapters: ["Dubai · Marina", "Dubai · JLT"] },
    ] },
    { name: "Abu Dhabi Region", code: "DEMO-R-AUH", zones: [
      { name: "Corniche Zone", code: "DEMO-Z-CORN", chapters: ["Abu Dhabi · Corniche", "Abu Dhabi · Al Maryah"] },
      { name: "Yas Zone", code: "DEMO-Z-YAS", chapters: ["Abu Dhabi · Yas Island"] },
    ] },
    { name: "Northern Emirates Region", code: "DEMO-R-NE", zones: [
      { name: "Sharjah Zone", code: "DEMO-Z-SHJ", chapters: ["Sharjah · Al Majaz", "Sharjah · Aljada"] },
...
const CHAPTER_OFFICER_ROLES = [
  ["President", "president"], ["Vice President", "vice_president"], ["Secretary", "secretary"],
  ["Treasurer", "treasurer"], ["VP Membership", "vp_membership"], ["VP Programming", "vp_programming"],
...
async function insertUsersBatch(rows: (typeof schema.users.$inferInsert)[]): Promise<number[]> {
  if (!rows.length) return [];
  const db = getDb();
  await db.insert(schema.users).values(rows);
  const unionIds = rows.map((r) => r.unionId as string);
  const got = await db.select({ id: schema.users.id, unionId: schema.users.unionId }).from(schema.users).where(inArray(schema.users.unionId, unionIds));
...
export async function loadFullDemo(): Promise<{ loaded: boolean; members: number; chapters: number; admins: number }> {
  ...
  let uSeq = 0;
  const email = (name: string) => `${name.toLowerCase().replace(/[^a-z]+/g, ".")}.${++uSeq}@demo.ehive.ae`;

  // ---- management team (admins) ----
  const mgmtRows = MANAGEMENT.map((m, i) => ({
    unionId: `seed-mgmt-${i}`, email: email(m.name), name: m.name,
    passwordHash: hash, role: "admin" as const, adminScopes: m.scopes,
  }));
  ...
  for (const region of HIERARCHY.regions) {
    ...
    for (const zone of region.zones) {
      const zoneId = Number((await db.insert(schema.orgUnits).values({ level: "zone", name: zone.name, code: zone.code, parentId: regionId }))[0].insertId);
      const zoneLeaderCandidates: number[] = [];

      for (const chapterName of zone.chapters) {
        chapterTotal++;
        const chapterId = Number((await db.insert(schema.chapters).values({
          name: `eHive ${chapterName}`, code: `DEMO-CH-${chapterTotal}`, zoneId,
          country: "United Arab Emirates", region: region.name, zone: zone.name,
          status: weighted(rand, [["chartered", 6], ["mature", 2], ["provisional", 2]]),
          meetingCadence: "Bi-weekly · Tue 7:30pm",
        }))[0].insertId);

        const count = 30 + Math.floor(rand() * 11); // 30–40
        const userRows: (typeof schema.users.$inferInsert)[] = [];
        const meta: { name: string; tier: "horizon" | "ascent" | "vanguard" | "zenith"; lc: string; score: number; company: string; title: string; sector: string }[] = [];
        for (let i = 0; i < count; i++) {
          const name = `${pick(rand, FIRST)} ${pick(rand, LAST)}`;
          userRows.push({ unionId: `seed-fd-${chapterId}-${i}`, email: email(name), name, passwordHash: hash, role: "user", adminScopes: "" });
          meta.push({
            name,
            tier: weighted(rand, [["horizon", 40], ["ascent", 40], ["vanguard", 17], ["zenith", 3]]),
            lc: weighted(rand, [["active", 68], ["onboarding", 10], ["at_risk", 8], ["renewal", 7], ["lapsed", 4], ["alumni", 3]]),
            score: 25 + Math.floor(rand() * 70),
            company: pick(rand, COMPANIES), title: pick(rand, TITLES), sector: pick(rand, SECTORS),
          });
        }
        ...
        const memberRows = userIds.map((uid, i) => ({
          userId: uid, tier: meta[i].tier, status: "active" as const,
          lifecycleState: meta[i].lc as never, hiveScore: meta[i].score,
          company: meta[i].company, title: meta[i].title, sector: meta[i].sector,
          homeChapterId: chapterId, renewalAt: renewal,
        }));
        await db.insert(schema.members).values(memberRows);
        const gotM = await db.select({ id: schema.members.id, userId: schema.members.userId }).from(schema.members).where(inArray(schema.members.userId, userIds));
        ...
        const officerRows = CHAPTER_OFFICER_ROLES.map(([title, role], i) => ({
          chapterId, memberId: memberIds[i], role, title, status: "active" as const, appointedBy: "demo",
        }));
        await db.insert(schema.chapterRoles).values(officerRows);

        // a health snapshot so roll-ups show a band
        const t = weighted(rand, [[82, 4], [68, 4], [55, 2]]);
        await db.insert(schema.healthSnapshots).values({
          chapterId, total: t, retention: Math.round(t * 0.25), engagement: Math.round(t * 0.25),
      ...
      if (zoneLeaderCandidates.length) {
        await db.insert(schema.unitRoles).values({ unitId: zoneId, level: "zone", memberId: zoneLeaderCandidates[0], role: "Zone Chair" });
      }
    }

    // regional director + deputy
    if (regionLeaderCandidates.length) {
      await db.insert(schema.unitRoles).values({ unitId: regionId, level: "region", memberId: regionLeaderCandidates[0], role: "Regional Director" });
      if (regionLeaderCandidates[1]) await db.insert(schema.unitRoles).values({ unitId: regionId, level: "region", memberId: regionLeaderCandidates[1], role: "Regional VP" });
    }
  }
  ...
  await db.insert(schema.appConfig).values({ key: DEMO_FULL_MARKER, value: new Date().toISOString() })
    .onDuplicateKeyUpdate({ set: { value: new Date().toISOString() } });

  return { loaded: true, members: memberTotal, chapters: chapterTotal, admins: MANAGEMENT.length };
}
...
export async function removeDemoData(): Promise<Record<string, number>> {
  ...
  const demoChapters = await db.select({ id: schema.chapters.id }).from(schema.chapters)
    .where(or(inArray(schema.chapters.code, SEED_CHAPTER_CODES), like(schema.chapters.code, "DEMO-%")));
  const chapterIds = demoChapters.map((c) => c.id);
  // ---- demo org units (old exact names + full-demo DEMO- codes) ----
  const demoUnits = await db.select({ id: schema.orgUnits.id }).from(schema.orgUnits)
    .where(or(
      like(schema.orgUnits.code, "DEMO-%"),
      ...SEED_ORG_UNITS.map((u) => and(eq(schema.orgUnits.level, u.level), eq(schema.orgUnits.name, u.name))),
    ));
  ...
  if (memberIds.length || unitIds.length) {
    const conds = [];
    if (memberIds.length) conds.push(inArray(schema.unitRoles.memberId, memberIds));
    if (unitIds.length) conds.push(inArray(schema.unitRoles.unitId, unitIds));
    await del("unitRoles", (await db.delete(schema.unitRoles).where(or(...conds)))[0].affectedRows);
  }

  // seeded org hierarchy
  if (unitIds.length) {
    await del("orgUnits", (await db.delete(schema.orgUnits).where(inArray(schema.orgUnits.id, unitIds)))[0].affectedRows);
  }

  // clear the full-demo marker so it can be loaded again
  await db.delete(schema.appConfig).where(eq(schema.appConfig.key, DEMO_FULL_MARKER));
