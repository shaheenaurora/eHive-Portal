...
async function openCaseFor(memberId: number) {
  const rows = await getDb()
    ...
    .where(and(eq(schema.memberSaveCases.memberId, memberId), inArray(schema.memberSaveCases.status, OPEN_STATES)))
    .limit(1);
  return rows.at(0);
}
...
export async function openSaveCase(memberId: number, reason: string, chapterId?: number | null): Promise<number> {
  const existing = await openCaseFor(memberId);
  if (existing) return existing.id;
  const res = await getDb().insert(schema.memberSaveCases).values({
    memberId,
...
export async function autoCloseSaveOnRecovery(memberId: number, note = "Re-engaged — engagement standard met again."): Promise<boolean> {
  const existing = await openCaseFor(memberId);
...
export type SaveCaseRow = {
  ...
  chapterId: number | null; chapterName: string | null;
...
export async function listSaveCases(opts: { status?: "open" | "closed" | "all" } = {}): Promise<SaveCaseRow[]> {
  ...
  const rows = await getDb()
    ...
    .from(schema.memberSaveCases)
    .leftJoin(schema.members, eq(schema.members.id, schema.memberSaveCases.memberId))
    .leftJoin(schema.users, eq(schema.users.id, schema.members.userId))
    .leftJoin(schema.chapters, eq(schema.chapters.id, schema.memberSaveCases.chapterId))
    .leftJoin(owner, eq(owner.id, schema.memberSaveCases.ownerUserId))
    .orderBy(desc(schema.memberSaveCases.openedAt))
    .limit(500);

  return rows
    .filter((r) => wanted === "all" || (wanted === "open" ? OPEN_STATES.includes(r.status) : !OPEN_STATES.includes(r.status)))
    .map((r) => ({
      ...r,
...
export async function updateSaveCase(
  ...
  if (Object.keys(set).length) {
    await getDb().update(schema.memberSaveCases).set(set).where(
      and(eq(schema.memberSaveCases.id, id), inArray(schema.memberSaveCases.status, OPEN_STATES)),
    );
  }
}
...
export async function closeSaveCase(id: number, outcome: "saved" | "lost", resolution?: string): Promise<number | null> {
  const db = getDb();
  const rows = await db.select().from(schema.memberSaveCases).where(eq(schema.memberSaveCases.id, id)).limit(1);
  const c = rows.at(0);
  if (!c || !OPEN_STATES.includes(c.status)) return null;
  await db.update(schema.memberSaveCases)
    .set({ status: outcome, resolution: resolution ?? null, closedAt: new Date() })
  ...
  if (outcome === "saved") {
    ...
    await db.update(schema.members)
      ...
      .where(and(eq(schema.members.id, c.memberId), eq(schema.members.lifecycleState, "at_risk")));
  }
  return c.memberId;
}
