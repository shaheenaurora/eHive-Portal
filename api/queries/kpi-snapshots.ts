...
export async function captureKpiSnapshots(
  now = new Date()
  ...
  await db
    ...
    .where(eq(schema.kpiSnapshots.capturedOn, on));
...
export async function kpiTrends(
  ...
  const rows = await db
    .select({
      metric: schema.kpiSnapshots.metric,
      value: schema.kpiSnapshots.value,
      capturedOn: schema.kpiSnapshots.capturedOn,
    })
  ...
  for (const r of rows)
    (out[r.metric] ??= []).push({ capturedOn: r.capturedOn, value: r.value });
...
export async function metricDelta(
  metric: string
  ...
  const rows = await getDb()
    ...
    .where(
      and(
        eq(schema.kpiSnapshots.scope, "network"),
        eq(schema.kpiSnapshots.metric, metric)
      )
    )
...
export async function latestChapterChiSnapshots(): Promise<
  Map<number, number>
> {
  const rows = await getDb()
    .select({
      scopeId: schema.kpiSnapshots.scopeId,
      value: schema.kpiSnapshots.value,
      capturedOn: schema.kpiSnapshots.capturedOn,
    })
    .from(schema.kpiSnapshots)
    .where(
      and(
        ...
        eq(schema.kpiSnapshots.metric, "chi")
      )
    )
    .orderBy(desc(schema.kpiSnapshots.capturedOn));
  const m = new Map<number, number>();
  for (const r of rows) {
    if (r.scopeId != null && !m.has(r.scopeId)) m.set(r.scopeId, r.value);
  }
  return m;
}

/** Used by removeDemoData-style cleanups if ever needed. */
export async function purgeSnapshotsForChapters(
  chapterIds: number[]
): Promise<void> {
