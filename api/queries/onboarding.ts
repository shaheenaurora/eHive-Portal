...
export async function computeOnboarding(
  ...
  const [attended, buddy, pod, session, manualRows] = await Promise.all([
    db
      ...
      .where(
        and(
          eq(schema.eventRegs.memberId, mid),
    ...
    db
      .select({ n: sql<number>`count(*)` })
      .from(schema.buddies)
      .where(eq(schema.buddies.newMemberId, mid)),
    db
      .select({ n: sql<number>`count(*)` })
    ...
    db
      ...
      .where(
        and(
          eq(schema.attendance.memberId, mid),
    ...
    db
      ...
      .where(eq(schema.onboardingMilestones.memberId, mid)),
  ]);
  ...
  const dayCount = Math.max(
    0,
    Math.floor((Date.now() - new Date(member.joinedAt).getTime()) / DAY)
  );
  ...
  return {
    ...
    dayCount: Math.min(dayCount, ONBOARDING_DAYS + 999),
    stage,
