...
async function latestHealthMap(
  chapterIds: number[]
...
export async function chaptersOverview() {
  ...
  const chapters = await db
    .select({ id: schema.chapters.id, status: schema.chapters.status })
  ...
  const [memberCount] = await db
    ...
    .from(schema.members)
    .where(
      sql`${schema.members.status} = 'active' and ${schema.members.homeChapterId} is not null`
    );
  ...
  const avgChi = chis.length
    ? Math.round(chis.reduce((a, b) => a + b, 0) / chis.length)
    : null;
  const atBar = chis.length
    ? Math.round((chis.filter(v => v >= 65).length / chis.length) * 100)
    : 0;
...
export async function chapterActivity(chapterId: number): Promise<Activity[]> {
  const db = getDb();
  const [snaps, elections, motions, budgets, meetings, roles] =
    await Promise.all([
      db
        .select()
        .from(schema.healthSnapshots)
        .where(eq(schema.healthSnapshots.chapterId, chapterId))
        .orderBy(desc(schema.healthSnapshots.createdAt))
        .limit(20),
      db
        .select()
      ...
      db
        .select({
          ...
          status: schema.chapterRoles.status,
          createdAt: schema.chapterRoles.createdAt,
          termEnd: schema.chapterRoles.termEnd,
        ...
        .where(eq(schema.chapterRoles.chapterId, chapterId))
        .orderBy(desc(schema.chapterRoles.createdAt))
        .limit(30),
    ]);
  ...
  A.push(
    snaps.map(s => ({
      ...
      kind: "health",
      icon: "✦",
      title: `Health snapshot · index ${s.total}`,
    }))
  );
  A.push(
    elections.map(e => ({
      ...
      kind: "election",
      icon: "⚖",
      title: `Election: ${e.title}`,
      detail: `${e.seat} · ${e.status}`,
    }))
  );
  A.push(
    motions.map(m => ({
  ...
  A.push(
    budgets.map(b => ({
      ...
      kind: "budget",
      icon: "▦",
      title: `Budget ${b.kind}: ${b.label}`,
      detail: `AED ${b.amount.toLocaleString()} · ${b.status}`,
    }))
  );
  A.push(
    meetings.map(m => ({
      ...
      kind: "meeting",
      icon: "◷",
      title: `${m.kind.replace("_", " ")}: ${m.title}`,
      detail: m.status,
    }))
  );
  A.push(
    roles.map(r => ({
      ...
      kind: "role",
      icon: "★",
      title: `Role ${r.status === "active" ? "assigned" : "ended"}: ${r.title || r.role}`,
      actor: r.name ?? undefined,
    }))
  );
