...
export async function listCouncil(unitId: number) {
  ...
  const [meetings, decisions] = await Promise.all([
    db
      ...
      .orderBy(
        desc(schema.councilMeetings.scheduledAt),
        desc(schema.councilMeetings.createdAt)
      )
      .limit(100),
    db
      .select()
      ...
      .orderBy(desc(schema.councilDecisions.createdAt))
      .limit(200),
  ]);
...
export async function createCouncilMeeting(
  unitId: number,
  input: { title: string; scheduledAt?: Date | null; agenda?: string | null }
): Promise<number> {
...
export async function logDecision(
  unitId: number,
  input: { meetingId?: number | null; title: string; detail?: string | null }
): Promise<number> {
  const res = await getDb()
...
export async function updateDecision(
  id: number,
  status: "proposed" | "carried" | "failed" | "deferred"
): Promise<void> {
  await getDb()
    .update(schema.councilDecisions)
