...
if (
  ...
  (!/test/i.test(dbName) && process.env.ALLOW_ITEST !== "1")
) {
  console.error(
    "Refusing to run: point DATABASE_URL at a *test* database (name containing 'test') or set ALLOW_ITEST=1. This script writes data."
  );
  process.exit(2);
}
...
const bad = (n: string, e?: unknown) => {
  fail++;
  console.log(
    "  \x1b[31mFAIL\x1b[0m " + n + (e ? "  → " + (e as Error).message : "")
  );
};
...
async function expectErr(
  ...
  try {
    await fn();
    bad(name, new Error("expected an error, got success"));
  } catch (e) {
    const m = (e as Error).message || "";
    if (match && !match.test(m)) bad(name, new Error("wrong error: " + m));
    else ok(name);
  }
}
...
function caller(user: Partial<schema.User> | undefined): Caller {
  return appRouter.createCaller({
    ...
    user: user as schema.User | undefined,
  });
}
...
async function main() {
  ...
  await check("saveChapter with geo hierarchy", async () => {
    await MA.adminEngage.saveChapter({
      name: "Dubai " + uniq,
      code: "AE-DXB",
      country: "UAE",
      region: "Gulf",
    ...
    const d = list.find(c => c.name === "Dubai " + uniq);
    const b = list.find(c => c.name === "AbuDhabi " + uniq);
    assert(!!d && d.zone === "DIFC" && d.code === "AE-DXB", "geo not stored");
    chA = d!.id;
    chB = b!.id;
  });
  await expectErr(
    "events-admin cannot saveChapter (scope)",
    () => caller(eventsAdmin).adminEngage.saveChapter({ name: "X" }),
    /doesn't include/i
  );
  ...
  let mApplicant = 0;
  await check(
    "admission creates member in Onboarding, in the chapter",
    async () => {
      const ap = await db.insert(schema.applications).values({
        userId: uApplicant.id,
        name: "Applicant",
        email: uApplicant.email,
        tierRequested: "ascent",
        status: "received",
      });
      const r = await MA.admin.setApplicationStatus({
        id: Number(ap[0].insertId),
        status: "approved",
        tier: "ascent",
        chapterId: chA,
      });
      ...
      const m = (
        await db
          .select()
          .from(schema.members)
          .where(eq(schema.members.id, mApplicant))
          .limit(1)
      )[0];
      assert(
        m.lifecycleState === "onboarding" && m.homeChapterId === chA,
        "not onboarding-in-chapter"
      );
    }
  );
  await check(
    "lifecycle onboarding→active→at_risk→active→renewal→lapsed→alumni w/ status coherence",
    async () => {
      ...
      let m = (
        await db
          .select()
          .from(schema.members)
          .where(eq(schema.members.id, mMem2))
          .limit(1)
      )[0];
      ...
      await db
        ...
        .set({ lifecycleState: "active", status: "active" })
        .where(eq(schema.members.id, mMem2));
    }
  );
  await expectErr(
    "plain member cannot drive lifecycle (RBAC)",
    () =>
      caller(uPres).admin.setLifecycleState({
        memberId: mMem3,
        state: "suspended",
      }),
    /permission|admin|UNAUTHORIZED|FORBIDDEN/i
  );

  console.log("\n\x1b[1mC. Chapter transfers (approval)\x1b[0m");
  await check("member requests transfer → pending", async () => {
    await caller(uOther).circle.requestChapterTransfer({
      toChapterId: chA,
      note: "relocated",
    });
  ...
  await expectErr(
    ...
    () => caller(uOther).circle.requestChapterTransfer({ toChapterId: chA }),
    /already have/i
  );
  await check("admin approves transfer → home chapter moves", async () => {
    ...
    await MA.adminEngage.decideChapterTransfer({
      id: req!.req.id,
      decision: "approve",
    });
    const m = (
  ...
  await check("assign President to a chapter member", async () => {
    ...
    assert(
      det.board.some(b => b.role === "president" && b.memberId === mPres),
      "no president"
    );
  });
  await expectErr(
    "cannot assign role to a non-member of the chapter",
    () =>
      MA.adminEngage.assignChapterRole({
        chapterId: chA,
        memberId: mFree,
        role: "secretary",
      }),
    /member of this chapter/i
  );
  await check(
    "re-assigning President retires the previous holder (one active)",
    async () => {
      await MA.adminEngage.assignChapterRole({
        chapterId: chA,
        memberId: mMem3,
      ...
      const active = (
        await db
          ...
          .where(eq(schema.chapterRoles.chapterId, chA))
      ).filter(r => r.role === "president" && r.status === "active");
      assert(
        active.length === 1 && active[0].memberId === mMem3,
        "not exactly one active president"
      );
      await MA.adminEngage.assignChapterRole({
        chapterId: chA,
        memberId: mPres,
  ...
  await check("officer (President) can open the console", async () => {
    ...
    assert(
      !!ov.chapter &&
        ov.roleKeys.includes("president") &&
        typeof ov.health.total === "number",
      "no officer context"
    );
  });
  await expectErr(
    "non-officer member is denied the console (RBAC)",
    () => caller(uMem2).officer.overview(),
    /leadership role|don't lead/i
  );
  await check(
    "officer signs up an unassigned member into the chapter",
    async () => {
      ...
      const m = (
        await db
          .select()
          .from(schema.members)
          .where(eq(schema.members.id, mFree))
          .limit(1)
      )[0];
  ...
  await expectErr(
    "officer cannot poach a member of another chapter",
    () => caller(uPres).officer.signupMember({ memberId: mOther }),
    /another chapter/i
  );
  await check("officer assigns a mentor within the chapter", async () => {
    await caller(uPres).officer.assignMentor({
      menteeId: mFree,
      mentorId: mMem2,
    });
    const b = await db
      .select()
      .from(schema.buddies)
      .where(eq(schema.buddies.newMemberId, mFree));
    assert(
      b.some(x => x.buddyMemberId === mMem2),
      "no mentor pairing"
    );
  });
  await expectErr(
    "self-mentoring rejected",
    () =>
      caller(uPres).officer.assignMentor({ menteeId: mFree, mentorId: mFree }),
    /themselves/i
  );
  await check("officer posts a learning, members can read it", async () => {
    await caller(uPres).officer.postLearning({
      title: "Filling seats " + uniq,
      body: "Invite 2 guests/week.",
    });
    const mine = await caller(uMem2).engage.myChapter();
    assert(
      (mine.learnings ?? []).some(l => l.title === "Filling seats " + uniq),
      "learning not visible"
    );
  });

  console.log("\n\x1b[1mE. Temporal integrity\x1b[0m");
  const evFuture = Number(
    (
  ...
  await check("member registers for a future event", async () => {
    ...
    const reg = (
      await db
        ...
        .where(eq(schema.eventRegs.eventId, evFuture))
    )[0];
    assert(!!reg && reg.status === "registered", "not registered");
  });
  await expectErr(
    "cannot register for an event that already started",
    () => caller(uPres).circle.registerEvent({ eventId: evPast }),
    /already started/i
  );
  await expectErr(
    "cannot check in before the event window opens",
    async () => {
      const reg = (
        await db
          .select()
      ...
      return caller(uPres).engage.checkinEvent({
        eventId: evFuture,
  ...
  await expectErr(
    "events-admin cannot Mark attended before the event starts",
    () =>
      caller(eventsAdmin).admin.markEventAttendance({
        eventId: evFuture,
        memberId: mPres,
        status: "attended",
      }),
  ...
  await check("check-in inside the window records attendance", async () => {
    await db
      ...
      .where(eq(schema.events.id, evFuture));
    const reg = (
      await db
        .select()
        .from(schema.eventRegs)
    ...
    await caller(uPres).engage.checkinEvent({
      eventId: evFuture,
      code: reg.checkinCode!,
    });
    const after = (
  ...
  await check(
    "buddy 30-day check-in blocked early, allowed after 40 days",
    async () => {
      const bId = Number(
        (
      ...
      try {
        ...
        throw new Error("early check-in should have failed");
      } catch (e) {
        if (!/opens closer/i.test((e as Error).message)) throw e;
      }
      await db
        .update(schema.buddies)
        .set({ pairedAt: new Date(Date.now() - 40 * 86400000) })
        .where(eq(schema.buddies.id, bId));
      await caller(uPres).engage.buddyCheckin({ id: bId });
      assert(
        !!(
          await db
  ...
  await check("member upgrade request is pending, tier unchanged", async () => {
    const before = (
      await db
        .select()
        .from(schema.members)
        .where(eq(schema.members.id, mPres))
        .limit(1)
    )[0];
    await caller(uPres).circle.requestMembershipChange({
      type: "upgrade",
      toTier: "vanguard",
    });
    const m = (
    ...
    assert(
      !!(await caller(uPres).circle.pendingTierRequest()),
      "no pending request"
    );
  });
  await expectErr(
    "duplicate tier request blocked",
    () =>
      caller(uPres).circle.requestMembershipChange({
        type: "upgrade",
        toTier: "zenith",
      }),
    /awaiting approval/i
  ...
  await check("admin approves tier request → tier changes", async () => {
    const req = (await MA.admin.pendingTierRequests()).find(
      r => r.req.memberId === mPres
    );
  ...
  await check(
    "health index computes with all six components in range",
    async () => {
      ...
      assert(
        [
          ...
          "leadership",
          "governance",
        ].every(k => k in h.components),
        "missing components"
      );
      assert(
        h.total >= 0 && h.total <= 100 && h.memberCount >= 3,
        "total/count wrong"
      );
  ...
  await check("save + read a quarterly snapshot", async () => {
    ...
    assert(
      !!hv.lastSnapshot &&
        hv.lastSnapshot.total === (r as { total: number }).total,
  ...
  await check("officer sets up the standard cadences", async () => {
    ...
    const ov = await caller(uPres).officer.overview();
    assert(
      ov.cadence.cadences.some(c => c.type === "chapter_meeting"),
      "no chapter meeting cadence"
    );
    assert(
      ov.cadence.cadences.every(c => c.currentStatus === "open"),
      "should start open"
    );
  });
  ...
  await check("marking a cadence kept updates its current status", async () => {
    ...
    await caller(uPres).officer.markCadence({
      cadenceId: cad.id,
      status: "kept",
    });
    const ov2 = await caller(uPres).officer.overview();
    assert(
      ov2.cadence.cadences.find(c => c.id === cad.id)!.currentStatus === "kept",
      "not kept"
    );
  });
  await expectErr(
    "officer of another chapter cannot mark this chapter's cadence",
    async () => {
      const ov = await caller(uPres).officer.overview();
      // uMem2 is not an officer at all → denied at requireOfficer
      return caller(uMem2).officer.markCadence({
        cadenceId: ov.cadence.cadences[0].id,
        status: "kept",
      });
    },
    /leadership role|don't lead/i
  ...
  await check("myOnboarding returns the ten staged milestones", async () => {
    ...
    assert(
      p.milestones.length === 10 && p.lifecycleState === "onboarding",
      "wrong milestone set"
    );
    assert(
      p.milestones.some(m => m.stage === 1) &&
        p.milestones.some(m => m.stage === 3),
      "no stages"
    );
  });
  await expectErr(
    "cannot check off an auto-tracked step",
    () =>
      caller(uApplicant).circle.completeOnboardingStep({
        milestone: "profile_complete",
      }),
    /tracked automatically/i
  );
  await check("manual step records and lifts progress", async () => {
    ...
    await caller(uApplicant).circle.completeOnboardingStep({
      milestone: "ask_offer",
    });
  ...
  await check("completing every milestone auto-confirms Active", async () => {
    ...
    await db
      .update(schema.members)
      .set({ company: "Acme", title: "Founder" })
      .where(eq(schema.members.id, mApplicant));
    const ev = Number(
      (
        await db.insert(schema.events).values({
    ...
    await db
      .insert(schema.eventRegs)
      .values({ eventId: ev, memberId: mApplicant, status: "attended" });
    await db.insert(schema.buddies).values({
      newMemberId: mApplicant,
      buddyMemberId: mPres,
      pairedAt: new Date(),
    });
    const pod = Number(
      (await db.insert(schema.pods).values({ name: "Pod X", kind: "pod" }))[0]
        .insertId
    );
    await db
      .insert(schema.podMembers)
      .values({ podId: pod, memberId: mApplicant });
    const ses = Number(
      (
        await db
          .insert(schema.sessions)
          .values({ podId: pod, startsAt: new Date(Date.now() - 3600000) })
      )[0].insertId
    );
    await db
      .insert(schema.attendance)
      .values({ sessionId: ses, memberId: mApplicant, status: "attended" });
  ...
  await check(
    "POD content is withheld until confidentiality is accepted",
    async () => {
      const before = await caller(uMem2).circle.podDetail({ id: podId });
      assert(
        before.confidentialityAccepted === false,
        "should start not accepted"
      );
      ...
      assert(
        stillHidden.sessions.length === 0,
        "sessions leaked before acceptance"
      );
      await caller(uMem2).circle.acceptPodConfidentiality({ podId });
      const after = await caller(uMem2).circle.podDetail({ id: podId });
      assert(
        after.confidentialityAccepted === true && after.sessions.length === 1,
        "content not revealed after acceptance"
      );
    }
  );
  await expectErr(
    "a non-member cannot open a POD's space",
    () => caller(uPres).circle.podDetail({ id: podId }),
    /Not in this pod|FORBIDDEN/i
  );
  await check("matching engine ranks pods and blocks conflicts", async () => {
    await db
      .update(schema.members)
      .set({ company: "Acme", sector: "FinTech" })
      .where(eq(schema.members.id, mMem3));
    await db
      .update(schema.members)
      .set({ company: "Acme" })
      .where(eq(schema.members.id, mMem2)); // same company as mMem3, already in pod
    const sug = await caller(owner).admin.suggestPodPlacement({ id: mMem3 });
    const row = sug.find(s => s.podId === podId);
    assert(
      !!row && row.blocked === "competition",
      "non-competition not enforced"
    );
  });
  await check("POD health computes from attendance + commitments", async () => {
    const pd = await caller(owner).admin.podAdmin({ id: podId });
    assert(
      pd.health && pd.health.total >= 0 && pd.health.total <= 100,
      "no pod health"
    );
  });
  ...
  await expectErr(
    "unauthenticated cannot list members",
    () => caller(undefined).admin.members(),
    /Authentication|UNAUTHORIZED/i
  );
  await expectErr(
    "member cannot approve applications",
    () =>
      caller(uPres).admin.setApplicationStatus({ id: 1, status: "approved" }),
    /permission|admin|FORBIDDEN|UNAUTHORIZED/i
  );
