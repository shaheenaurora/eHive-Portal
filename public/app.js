...
(function () {
  ...
  (function () {
    ...
    var ACT_NAME = {
      ...
      trading: "trading & general trading",
    ...
    function estimate(j) {
      var r = RATE[j];
      var offKey = office.value,
        offCost = OFFICE[offKey];
      if (j === "ml" && offKey === "none") {
        offCost = OFFICE.flexi;
      }
      ...
      return {
        low: round500(total * 0.95),
        high: round500(total * 1.25),
        base: base,
        visaCost: visaCost,
        offCost: offCost,
        mult: ACT[activity.value],
        visas: v,
      };
    }
    var lastMain = "fz",
    ...
    function render() {
      ...
      if (e.mult !== 1) {
        rows +=
          "<li><span>Activity-specific requirements</span><b>\u00d7 " +
          e.mult.toFixed(2) +
          "</b></li>";
      }
      ...
      if (jur === "both") {
        ...
        coAlt.hidden = false;
        coAlt.innerHTML =
          "Mainland path for the same setup: <b>AED " +
          fmt(m.low) +
          " – " +
          fmt(m.high) +
          "</b>";
    ...
    [activity, office].forEach(function (el) {
        ...
        submitLead(
          {
            ...
            email: em.value.trim(),
            estimate: {
              jurisdiction: RATE[lastMain].name,
            ...
            source_page: "business-setup.html",
          ...
          function () {
            ...
            if (ok) {
              ...
              ok.textContent =
                "Sent — the full breakdown is on its way to your inbox.";
            }
          ...
          function () {
            ...
            if (err) {
              ...
              err.textContent =
                "The breakdown service isn\u2019t connected yet (pre-launch). Note your estimate above — or send it through Get Started and we\u2019ll pick it up at launch.";
            }
  ...
  (function () {
    ...
    var PT = {
      ...
      ascent: "Ascent",
      vanguard: "Vanguard",
      zenith: "Zenith (invitation-only)",
    };
    var prog = document.getElementById("gsProg");

    function paneOf(step) {
      return step.getAttribute("data-pane");
    }
    function activeStep() {
    ...
    function currentValid() {
      ...
      if (n === "2") {
        ...
        if (p === "business") {
          var a = document.getElementById("gsActivity");
          return !!(a && a.value.trim());
        }
        if (p === "other") {
          var q = document.getElementById("gsQuestion");
    ...
    function paint() {
      ...
      if (prog) {
        prog.textContent =
          n === "done" ? "Request routed" : "Step " + num + " of 4";
      }
      var pane = paneOf(st);
      gsTitle.textContent =
        n === "1"
          ? "What brings you to eHive?"
          : n === "3"
      ...
      st.querySelectorAll(".gs-next").forEach(function (b) {
        b.disabled = !currentValid();
      });
    }
    function go(n, push) {
      ...
      if (n === "done") sel = '.gs-step[data-step="done"]';
        ...
        sel = '.gs-step[data-step="2"][data-pane="' + gs.door + '"]';
    ...
    document.querySelectorAll(".gs-path[data-door]").forEach(function (b) {
      b.addEventListener("click", function () {
        ...
        gs.detail = null;
        document.querySelectorAll(".gs-path[data-door]").forEach(function (x) {
    ...
    document.querySelectorAll(".gs-path[data-detail]").forEach(function (b) {
      b.addEventListener("click", function () {
        ...
        pane.querySelectorAll(".gs-path[data-detail]").forEach(function (x) {
          x.classList.remove("active");
        });
        b.classList.add("active");
        paint();
    ...
    document.querySelectorAll(".gs-back").forEach(function (b) {
      b.addEventListener("click", function () {
        if (b.getAttribute("data-back") === "2") go(2);
        else go(1);
      });
    });
    ...
    if (gsSubmit) {
      gsSubmit.addEventListener("click", function () {
        ...
        if (gs.door === "business") {
          ...
          detail =
            act + " · " + document.getElementById("gsJurisdiction").value;
        ...
        rows.forEach(function (r, i) {
          spans[i].textContent = r[0];
          bs[i].textContent = r[1];
        });
        ...
        submitLead(
          {
            ...
            door: gs.door,
            detail: detail,
            product_or_tier: productOrTier,
            question: question,
            name: name.value.trim(),
            email: email.value.trim(),
            phone: phone,
            source_page: "get-started.html",
            referrer: document.referrer || "",
            timestamp: new Date().toISOString(),
            user_agent: navigator.userAgent,
          },
    ...
    if (door && LBL[door]) {
      ...
      if (sel && PT[sel] && (door === "consulting" || door === "circle")) {
        ...
        var pb = document.querySelector(
          '.gs-path[data-detail="' + PT[sel] + '"]'
        );
        if (pb) pb.classList.add("active");
        go(3, false);
        try {
          history.replaceState({ gsStep: "3", door: door }, "", "#step-3");
        } catch (e) {}
      } else {
        go(2, false);
        try {
          history.replaceState({ gsStep: "2", door: door }, "", "#step-2");
        } catch (e) {}
      }
    } else {
      paint();
