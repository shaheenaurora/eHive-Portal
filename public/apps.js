...
(function () {
  ...
  function confetti(x, y, n) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      c.style.left = x + (Math.random() - 0.5) * 140 + "px";
      c.style.top = y - 10 + (Math.random() - 0.5) * 30 + "px";
  ...
  var CG = {
    ...
    grow: {
      ...
      chips: [
        ["A stuck decision", "g_decision"],
        ["Growth has stalled", "g_stalled"],
        ["The brand isn&rsquo;t landing", "g_brand"],
        ["Operations depend on me", "g_ops"],
      ],
    },
    g_decision: {
      ...
      chips: [
        ["See Clarity Sprint", "go:consulting-clarity-sprint.html"],
        ["Take the fit quiz", "pf"],
        ["Start over", "start"],
      ],
    },
    g_stalled: {
      ...
      chips: [
        ["See GapNavigator", "go:consulting-gapnavigator.html"],
        ["Take the fit quiz", "pf"],
        ["Start over", "start"],
      ],
    },
    g_brand: {
      ...
      chips: [
        ["See Brand 3D", "go:consulting-brand-3d.html"],
        ["Take the fit quiz", "pf"],
        ["Start over", "start"],
      ],
    },
    g_ops: {
      ...
      chips: [
        ["Book the discovery call", "go:book.html?product=opsblueprint"],
        ["Take the fit quiz", "pf"],
        ["Start over", "start"],
      ],
    },
    ...
    explore: {
      ...
      chips: [
        ["Start the pathfinder", "pf"],
        ["Read about eHive", "go:about.html"],
        ["Start over", "start"],
      ],
    },
  };
    ...
    function act(action, label) {
      ...
      if (action === "sim") {
        if (document.getElementById("hsSim")) {
          closePanel();
          document
            .getElementById("hsSim")
            .scrollIntoView({ behavior: "smooth", block: "center" });
        } else {
          appToast("Opening <b>eHive Circle</b>…");
          setTimeout(function () {
            location.href = "circle.html#tiers";
          }, 450);
        }
        return;
      }
  ...
  var PF_Q = [
    {
      ...
      opts: [
        ["idea", "Just an idea — not launched"],
  ...
  function pfResult(a) {
    ...
    if (a[1] === "legal") {
      ...
      if (a[0] === "growing" || a[0] === "scaling") {
        r = {
          ...
          ctas: [
            ["See Momentum90", "consulting-momentum90.html"],
            ["Book the discovery call", "book.html?product=momentum90"],
          ],
        };
      } else {
        r = {
          ...
          ctas: [
            ["See OpsBlueprint", "consulting-opsblueprint.html"],
            ["Book the discovery call", "book.html?product=opsblueprint"],
          ],
        };
      }
    } else {
      if (a[0] === "growing" || a[0] === "scaling") {
        r = {
          ...
          ctas: [
            ["See Strategy Sprint", "consulting-strategy-sprint.html"],
            ["Book this sprint", "book.html?product=strategy-sprint"],
          ],
        };
      } else {
        r = {
          ...
          ctas: [
            ["See Clarity Sprint", "consulting-clarity-sprint.html"],
            ["Book this sprint", "book.html?product=clarity-sprint"],
          ],
        };
      }
    }
    if (a[2] === "exploring")
      r.desc =
        "No rush — and when you&rsquo;re ready: " +
        r.desc.charAt(0).toLowerCase() +
        r.desc.slice(1);
    return r;
  }
    ...
    function paint() {
      ...
      if (step >= 3) {
        ...
        body.innerHTML =
          ...
          '<a class="btn btn-ghost" style="color:var(--ink-900);border-color:rgba(20,24,33,.3)" href="' +
          r.ctas[1][1] +
          '">' +
          r.ctas[1][0] +
          "</a>" +
      ...
      var html =
        '<p class="pfm-q">' + PF_Q[step].q + '</p><div class="pfm-opts">';
      PF_Q[step].opts.forEach(function (o) {
        html +=
          '<button type="button" class="pfm-opt" data-v="' +
          o[0] +
      ...
      if (step > 0)
        html +=
          '<button type="button" class="pfm-back">← Back a question</button>';
      body.innerHTML = html;
    ...
    function show() {
      ...
      input.addEventListener("input", function () {
        ...
        filtered = CMDK_ITEMS.filter(function (i) {
          return (
            i[0].toLowerCase().indexOf(q) !== -1 ||
            i[1].toLowerCase().indexOf(q) !== -1
          );
        });
        sel = 0;
        paintList();
  ...
  function hiveSim() {
    ...
    var LABELS = [
      [0, "Your first action starts the journey."],
      [10, "New-member badge earned — you&rsquo;re in the room."],
      [25, "Cross-tier rooms are open to you now."],
      [50, "Councils notice members at 50+."],
      [
        80,
        "Vanguard eligibility reached. That&rsquo;s the top of the public ladder.",
      ],
    ];
    function paint(animPts) {
      arc.style.strokeDashoffset = String(CIRC * (1 - score / 100));
      if (animPts) {
        var from = +num.textContent,
          t0 = null;
        if (reduceMotion) {
      ...
      miles.forEach(function (m) {
        ...
        if (score >= at && !m.classList.contains("hit")) {
          ...
          confettiAt(m, 12);
          appToast(
            "Unlocked: <b>" + m.querySelector("span").textContent + "</b>"
          );
        } else if (score < at) {
    ...
    root.querySelectorAll(".sim-act").forEach(function (b) {
      b.addEventListener("click", function () {
        ...
        if (score >= 100) {
          appToast(
            "You&rsquo;re at <b>100</b> — go home, you&rsquo;ve earned it."
          );
          return;
        }
  ...
  function journey() {
    ...
    function show(i, animate) {
      ...
      var apply = function () {
        card.querySelector("#jrKicker").textContent =
          "Step " + (i + 1) + " of 5";
    ...
    playBtn.addEventListener("click", function () {
      ...
      playing = setInterval(function () {
        if (idx >= 4) {
          stopPlay();
          appToast(
            "That&rsquo;s the whole journey — <b>you&rsquo;re operational</b>."
          );
          return;
        }
        show(idx + 1, true);
  ...
  function comparer() {
    ...
    btns.forEach(function (b) {
      b.addEventListener("click", function () {
        ...
        vals.forEach(function (v) {
          v.classList.add("swap-out");
        });
        setTimeout(
          function () {
            paint(b.getAttribute("data-jc"));
          },
          reduceMotion ? 0 : 150
        );
      });
    ...
    playBtn.addEventListener("click", function () {
      ...
      playing = setInterval(function () {
        if (idx >= 5) {
          stopPlay();
          appToast(
            "And that&rsquo;s a sprint day — <b>decision made, map in hand</b>."
          );
          return;
        }
        show(idx + 1, true);
      ...
      var mo = new MutationObserver(function (muts) {
        muts.forEach(function (m) {
          ...
          if (m.target.classList.contains("active")) {
            if (!m.target.classList.contains("step-in"))
              m.target.classList.add("step-in");
